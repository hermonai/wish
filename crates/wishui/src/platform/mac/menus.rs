...
use wishui_core::actions::StandardAction;
use wishui_core::keymap::Keystroke;
use wishui_core::platform::menu::{
...
extern "C" {
    ...
    fn make_wish_custom_menu_item(ctx: *mut c_void) -> id;
...
fn resolve_standard_action(action: StandardAction) -> StandardMenuItemProperties {
    ...
    match action {
        ...
        StandardAction::Quit => make("Quit Wish", "terminate:", cmd, "q"),
        StandardAction::Hide => make("Hide Wish", "hide:", cmd, "h"),
...
unsafe fn make_menu_item(menu_item: MenuItem) -> id {
    match menu_item {
        MenuItem::Custom(custom_menu_item) => {
            ...
            let nsmenu_item = make_wish_custom_menu_item(MenuItemData::into_context(data));
