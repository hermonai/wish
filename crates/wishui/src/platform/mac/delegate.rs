...
use std::path::Path;
use std::sync::Arc;
use wishui_core::clipboard::InMemoryClipboard;
use wishui_core::keymap::Keystroke;
use wishui_core::modals::{AlertDialog, ModalId};
use wishui_core::notification::{NotificationSendError, RequestPermissionsOutcome};
use wishui_core::platform::{
    Cursor, FilePickerCallback, FilePickerConfiguration, MicrophoneAccessState,
    SendNotificationErrorCallback, TerminationMode,
};
use wishui_core::ApplicationBundleInfo;
use wishui_core::{
...
impl platform::Delegate for AppDelegate {
    ...
    fn show_native_platform_modal(&self, id: ModalId, modal: AlertDialog) {
        ...
        unsafe {
            let _: () = msg_send![app::get_wish_app(), showModal: alert modalId: id];
        }
    }
