...
use wishui_core::rendering::{GPUBackend, GPUDeviceInfo, GPUDeviceType};
...
/// Returns whether or not the provided adapter is an unsupported Nvidia driver version for wishui
/// to render properly.
...
/// wishui to render properly. Currently, we limit this to "Intel UHD Graphics 620", but we do have
...
/// (Vulkan llvmpipe) adapter that may not work properly with wishui.
...
fn adapter_stability_sort_func(
    ...
    // Same with Nvidia drivers, though this is only an issue with a Wayland window server.
        ...
        log::warn!(
            ...
            https://wish.hermon.ai/docs/help/known-issues#linux-1",
