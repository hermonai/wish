...
#   {download_base_url}         — e.g. https://app.hermon.ai/download/cli
#   {channel}                   — stable | preview | dev
#   {install_dir}               — e.g. ~/.warp/remote-server
#   {binary_name}               — e.g. hermon | hermon-dev | hermon-preview
...
if [ -n "$staging_tarball_path" ]; then
  ...
  mv "$staging_tarball_path" "$tmpdir/hermon.tar.gz"
else
  ...
  if command -v curl >/dev/null 2>&1; then
    curl -fSL --connect-timeout 15 "$url" -o "$tmpdir/hermon.tar.gz"
