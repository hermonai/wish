...
pub fn node_installation_dir() -> Result<PathBuf> {
    ...
    Ok(wish_core::paths::data_dir().join("node").join(folder_name))
}
...
pub async fn install_npm(client: &http_client::Client) -> Result<PathBuf> {
    ...
    let node_containing_dir = wish_core::paths::data_dir().join("node");
...
pub async fn detect_system_node(path_env_var: impl AsRef<OsStr>) -> Result<()> {
    ...
    // via Wish's inherited env rather than the captured interactive PATH.
    #[cfg(windows)]
