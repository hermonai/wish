...
impl SelectedSettings {
    ...
    pub fn is_wish_drive_enabled(&self) -> bool {
        match self {
            SelectedSettings::AgentDrivenDevelopment {
                ui_customization, ..
            } => ui_customization
                .as_ref()
                .map(|ui| ui.show_wish_drive)
                .unwrap_or(true),
            SelectedSettings::Terminal {
                ui_customization, ..
            } => ui_customization
...
impl OnboardingStateModel {
    ...
    pub(crate) fn set_tools_panel_enabled(&mut self, enabled: bool, ctx: &mut ModelContext<Self>) {
        ...
        self.ui_customization.show_wish_drive = enabled;
        ctx.notify();
    }
    ...
    pub(crate) fn set_show_wish_drive(&mut self, value: bool, ctx: &mut ModelContext<Self>) {
        if self.ui_customization.show_wish_drive == value {
            return;
        }
        send_telemetry_from_ctx!(
            OnboardingEvent::SettingChanged {
        ...
        self.ui_customization.show_wish_drive = value;
        ctx.notify();
    }
