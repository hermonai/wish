...
impl GcpCredentials {
    ...
    pub fn federated(
        ...
        let cred_config_json =
            generate_gcp_credential_config(task_id, config, &wish_binary_path, output_file.path())?;
        safe_debug!(
...
///
/// `wish_binary_path` should be the absolute path to the current `wish` (Hermon CLI) executable.
fn generate_gcp_credential_config(
    task_id: &str,
    config: &GcpFederationConfig,
    wish_binary_path: &Path,
    output_file: &Path,
    ...
    let wish_binary_display = wish_binary_path.display().to_string();
    ...
    if wish_binary_display.contains(' ') {
        return Err(PrepareGcpCredentialsError::InvalidBinaryPath {
            path: wish_binary_path.to_path_buf(),
        });
    }
    if task_id.contains(' ') {
    ...
    let mut command = format!("{wish_binary_display} federate issue-gcp-token --run-id {task_id}");
