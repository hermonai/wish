...
fn basic_config_shape() {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "credential_source": {
                "executable": {
                    "command": "/usr/bin/wish federate issue-gcp-token --run-id task-42",
...
fn rejects_binary_path_with_spaces() {
    ...
    let result = generate_gcp_credential_config(
        "task-1",
        &config,
        Path::new("/path with spaces/wish"),
        Path::new("/tmp/out"),
    );
    assert!(matches!(
        result,
...
fn service_account_impersonation() {
    ...
    let result =
        generate_gcp_credential_config("t-1", &config, Path::new("/bin/wish"), Path::new("/tmp/out"))
            .unwrap();
    ...
    assert_eq!(
        ...
        json!({
            ...
            "credential_source": {
                "executable": {
                    "command": "/bin/wish federate issue-gcp-token --run-id t-1 --duration 1800s",
                    "timeout_millis": 30000,
                    "output_file": "/tmp/out"
                }
            },
...
fn no_duration_flag_when_lifetime_absent() {
    ...
    let result =
        generate_gcp_credential_config("t-3", &config, Path::new("/bin/wish"), Path::new("/tmp/out"))
            .unwrap();
    assert_eq!(
        result,
        json!({
            ...
            "credential_source": {
                "executable": {
                    "command": "/bin/wish federate issue-gcp-token --run-id t-3",
                    "timeout_millis": 30000,
                    "output_file": "/tmp/out"
                }
            }
