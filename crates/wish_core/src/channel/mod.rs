...
impl Channel {
    ...
    /// `--session-sharing-server-url` flags and their `WISH_*` env-var equivalents.
    ///
    ...
    pub fn cli_command_name(&self) -> &'static str {
        match self {
            Channel::Stable => "wish",
            Channel::Dev => "wish-dev",
            Channel::Preview => "wish-preview",
            Channel::Local => "wish-local",
            Channel::Integration => "wish-integration",
