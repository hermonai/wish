...
pub struct ChannelConfig {
    ...
    /// Configuration for talking to Wish's servers.
    pub server_config: WishServerConfig,
    /// Configuration for the Hermon platform (ambient agents, dashboards).
    pub hermon_config: HermonConfig,
...
#[derive(Debug, Deserialize, Serialize)]
pub struct WishServerConfig {
    /// The root URL for the standard server pool.
    pub server_root_url: Cow<'static, str>,
    /// The URL for the RTC server, which serves real-time updates for Wish Drive objects.
...
#[derive(Debug, Deserialize, Serialize)]
pub struct HermonConfig {
    /// Root URL for the Hermon platform dashboard.
    pub hermon_root_url: Cow<'static, str>,

    /// URL to use as the audience when issuing workload identity tokens. If [`None`], falls back
    /// to [`WishServerConfig::server_root_url`]. This exists so the audience is not overridden
...
impl HermonConfig {
    ...
    pub fn local_dev() -> Self {
        Self {
            hermon_root_url: "http://localhost:3000".into(),
            workload_audience_url: None,
        }
    }
}
