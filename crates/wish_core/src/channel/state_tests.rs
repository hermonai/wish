...
fn wss_becomes_https_and_strips_path() {
    let got = derive_http_origin_from_ws_url("wss://wish.hermon.ai/graphql/v2");
    assert_eq!(got.as_deref(), Some("https://wish.hermon.ai"));
}
...
fn unparseable_input_returns_none() {
    ...
    assert!(derive_http_origin_from_ws_url("https://wish.hermon.ai").is_none());
}
