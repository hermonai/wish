...
use crate::{
    channel::config::{
        ChannelConfig, HermonConfig, McpOAuthProviderConfig, RudderStackDestination,
        WishServerConfig,
    },
...
impl ChannelState {
    pub fn init() -> Self {
        ...
        let app_id = AppId::new("ai", "hermon", "WishOss");
        Self {
            channel,
    ...
    ///
    /// This is a debugging tool for isolating development instances of Wish, and is not
    /// supported in release builds.
    pub fn data_profile() -> Option<String> {
        if cfg!(debug_assertions) {
            std::env::var("WISH_DATA_PROFILE").ok()
        } else {
            None
        }
    }
    ...
    /// telemetry events. Builds like Wish OSS intentionally ship with
    ...
    /// ship crash reports. Builds like Wish OSS intentionally ship with
    ...
    /// served by the Wish RTC service (e.g. the agent event SSE stream).
    ///
    ...
    pub fn hermon_root_url() -> Cow<'static, str> {
        CHANNEL_STATE
            .lock()
            .config
            .hermon_config
    ...
    pub fn workload_audience_url() -> Cow<'static, str> {
        ...
        match &state.config.hermon_config.workload_audience_url {
            Some(url) => url.clone(),
            None => {
                drop(state);
...
fn app_id_from_bundle() -> Option<AppId> {
    ...
    unsafe {
        ...
        use wishui::platform::mac::utils::nsstring_as_str;
