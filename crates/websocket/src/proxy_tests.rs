...
fn no_proxy_suffix_with_dot() {
    ...
    clear_proxy_env();
    env::set_var("HTTPS_PROXY", "http://proxy:3128");
    env::set_var("NO_PROXY", ".hermon.ai");

    assert!(resolved_proxy_tls("sessions.app.hermon.ai").is_none());

    assert!(resolved_proxy_tls("hermon.ai").is_some()); // Exact "hermon.ai" != ".hermon.ai"
    assert!(resolved_proxy_tls("other.com").is_some());
    clear_proxy_env();
}
...
fn no_proxy_suffix_without_dot() {
    ...
    // "sessions.app.hermon.ai" ends with ".hermon.ai" → matches
    assert!(resolved_proxy_tls("sessions.app.hermon.ai").is_none());
    // Exact match too
    assert!(resolved_proxy_tls("hermon.ai").is_none());
    assert!(resolved_proxy_tls("nothermon.ai").is_some());
    clear_proxy_env();
}
