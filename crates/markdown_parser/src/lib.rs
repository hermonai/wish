...
/// This purposefully shadows the `Action` trait from `wishui`.
///
/// Since `wishui` depends on this crate, we can't depend on the `wishui::Action` trait directly.
/// Instead, we create a new trait with a blanket implementation that implicitly results
/// in any `wishui::Action` implementing this `Action`.
