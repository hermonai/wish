...
pub fn test_expand_command_aliases() {
    ...
    let result = wishui::r#async::block_on(expand_command_aliases(
    ...
    let result = wishui::r#async::block_on(expand_command_aliases("aliasForTest ", false, &ctx));
    ...
    #[cfg(not(feature = "v2"))]
    {
        ...
        let result = wishui::r#async::block_on(expand_command_aliases("test twelve ", false, &ctx));
        ...
        let result =
            wishui::r#async::block_on(expand_command_aliases("aliasForTest twelve ", false, &ctx));
        assert_eq!(result.expanded_command_line, "test one ");
        assert_eq!(result.tokens_from_command, vec!["test", "one"]);
        // Should be using the subcommand signature for completions
        assert_eq!(
