...
pub enum FederateCommand {
    /// Issue an identity token for the current Hermon agent. This can only be called within a running Hermon agent session.
    IssueToken(IssueTokenArgs),
    /// Issue an identity token for the current Hermon agent, in the format expected by Google Cloud's
