...
/// `ScheduleCommand` has a slightly unusual definition because we allow `hermon schedule` as
// a shorthand for `hermon schedule create`.
...
impl ScheduleCommand {
    /// Get the specific scheduling subcommand. Returns `None` if using the `hermon schedule` creation shorthand.
