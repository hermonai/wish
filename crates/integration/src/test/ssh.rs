...
macro_rules! generate_can_bootstrap_tmux_ssh_test_for_shell {
    ($fn_name:ident, $shell:literal, $install_tmux:literal) => {
        ...
        /// successfully.
        pub fn $fn_name() -> Builder {
            fn wishify(builder: Builder) -> Builder {
                builder
                    .with_step(enter_ssh_command($shell))
                    .with_step(wait_for_password_prompt(0 /*tab_idx*/, $shell))
                    .with_step(
            ...
            fn assert_wishification(builder: Builder) -> Builder {
                builder
            ...
            let builder = new_builder()
                ...
                .with_step(setup_gcloud_sdk());
            // Install Tmux
            let builder = wishify(builder).with_step(
            ...
            let builder = assert_wishification(builder).with_step(run_exit_command());

            // Validate we can Wishify when Tmux is already installed
            assert_wishification(wishify(builder))
        }
    };
}
