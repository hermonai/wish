...
use wishui::event::{Event, ModifiersState};
use wishui::integration::{TestStep, ARTIFACTS_DIR_ENV_VAR};

use crate::Builder;
use wish::integration_testing::step::new_step_with_default_assertions;
use wish::integration_testing::terminal::util::ExpectedExitStatus;
            ...
            move |_app, _window_id, _persisted_data| -> Pin<Box<dyn Future<Output = ()> + Send>> {
                Box::pin(async move {
                    let artifacts_root = std::env::var(ARTIFACTS_DIR_ENV_VAR)
                        ...
                        .unwrap_or_else(|_| {
                            std::env::temp_dir().join("wish_integration_test_artifacts")
                        });
