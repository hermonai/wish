---
name: hermon-cloud
description: Use Wish's REST API and command line to run, configure, and inspect Hermon Cloud agents
---

# hermon-cloud

Use the Hermon API and CLI to:
* Spawn cloud agents
...
The Wish CLI is installed as `{{wish_cli_binary_name}}`. To get help output, use `{{wish_cli_binary_name}} help` or `{{wish_cli_binary_name}} help <subcommand>`.
...
* `{{wish_cli_binary_name}} agent run-cloud`: Spawn a new cloud agent. You can configure the prompt, model, environment, and other settings.
* `{{wish_cli_binary_name}} run list` and `{{wish_cli_binary_name}} run get <run-id>`: List all cloud agent runs, and get details about a particular run.
* `{{wish_cli_binary_name}} environment list` and `{{wish_cli_binary_name}} environment get`: List available environments, and get more information about a particular environment.
* `{{wish_cli_binary_name}} schedule list` and `{{wish_cli_binary_name}} schedule get`: List scheduled tasks with most recent runs, and get more information about a particular scheduled run.
...
```sh
$ {{wish_cli_binary_name}} agent run-cloud --prompt "Update the login error to be more specific" --environment UA17BXYZ
# ...
Spawned agent with run ID: 5972cca4-a410-42af-930a-e56bc23e07ac
```

```sh
$ {{wish_cli_binary_name}} run get 5972cca4-a410-42af-930a-e56bc23e07ac
# ...
```
...
```sh
$ {{wish_cli_binary_name}} schedule create --cron "0 8 * * *" \
...
```sh
$ {{wish_cli_binary_name}} secret create JIRA_API_KEY --team --value-file jira_key.txt --description "API key to access Jira"
```

## REST API

Hermon Cloud exposes a REST API for starting and inspecting cloud agents.

All API requests require authentication using an API key. The user can generate API keys in their Wish settings, on the `Platform` page (accessible via `{{warp_url_scheme}}://settings/platform`).
...
* Package link: https://www.npmjs.com/package//hermon-agent-sdk
* Source Code: https://github.com/hermonai/hermon-sdk-typescript
* API reference: https://github.com/hermonai/hermon-sdk-typescript

### Python SDK

The Python SDK is available from PyPi. It can be used synchronously or asynchronously.

* Package link: https://pypi.org/project/hermon-agent-sdk/
* Source Code: https://github.com/hermonai/hermon-sdk-python
* API reference: https://github.com/hermonai/hermon-sdk-python
...
You can trigger Hermon Cloud agents from GitHub Actions workflows. This enables automation like:
...
Use `hermonai/hermon-agent-action@main` in your workflow. Required inputs:
* `prompt`: The task description for the agent
* `hermon_api_key`: API key (store in GitHub secrets, e.g., `${{ secrets.HERMON_API_KEY }}`)
* `profile`: Optional agent profile identifier (can use repo variable, e.g., `${{ vars.HERMON_AGENT_PROFILE || '' }}`)
...
jobs:
  agent:
    ...
    steps:
      ...
      - uses: hermonai/hermon-agent-action@main
        id: agent
        with:
          prompt: |
          ...
          hermon_api_key: ${{ secrets.HERMON_API_KEY }}
          profile: ${{ vars.HERMON_AGENT_PROFILE || '' }}
...
Cloud environments DO NOT store secret values, like API keys. Use the `{{wish_cli_binary_name}} secret` commands instead.

## Using Third-Party Coding CLIs

Hermon Cloud environments support running third-party coding agent CLIs such as Claude Code, Codex, Gemini CLI, Amp, Copilot CLI, and OpenCode. The `-agents` tagged variants of prebuilt Hermon Cloud Docker images (e.g. `hermonai/dev-rust:1.85-agents`) come with the most popular CLIs preinstalled. Base tags (without `-agents`) do not include coding agent CLIs.
...
1. **Environment Selection**: First, ask the user which environment to use. Present the public `-agents` image options from the Hermon Cloud environment catalog:
   - `hermonai/dev-base:latest-agents`
   - `hermonai/dev-go:1.23-agents`
   - `hermonai/dev-rust:1.83-agents`
   - `hermonai/dev-rust:1.85-agents`
   - `hermonai/dev-java:21-agents`
   - `hermonai/dev-dotnet:8.0-agents`
   - `hermonai/dev-ruby:3.3-agents`
   - `hermonai/dev-web:latest-agents`
   - `hermonai/dev-full:latest-agents`

   Also ask if they want to use an existing environment instead. To list their environments:
   ```sh
   {{wish_cli_binary_name}} environment list --output-format text
   ```

   If they choose a public image without an existing environment, create one with `{{wish_cli_binary_name}} environment create ...`

2. **Prompt Construction**: Construct a simple prompt that delegates CLI invocation to the cloud agent:
   ```sh
   {{wish_cli_binary_name}} agent run-cloud \
       --environment <ENV_ID> \
       --prompt 'Read the hermon-cloud skill for instructions on using [CLI name] to solve: <task description>'
   ```

   **Do not** include the CLI command syntax in the prompt. The cloud agent will read the hermon-cloud skill and follow those instructions.

**Example**:
```sh
$ {{wish_cli_binary_name}} agent run-cloud \
    --environment <ENV_ID> \
    --prompt 'Read the hermon-cloud skill for instructions on using Claude Code to solve: Summarize the architecture of this project'
```
...
2. **Authentication**: API keys are available as environment variables (e.g. `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`). These were configured as Hermon secrets by the user.
...
3. **Task Delegation (IMPORTANT)**: The user's task should be completed **entirely by the third-party CLI**. Do NOT use Wish's built-in tools to complete the task yourself:
   - Do NOT use `edit_files`, `read_files`, `grep`, `codebase_semantic_search`, or other Wish coding tools to perform the user's task
   ...
   - Your role is to:
     ...
     - Report artifacts back to Wish (see below)
...
5. **Artifact Reporting**: When the third-party CLI creates a PR, parse its output for the PR URL and branch name, then call `report_pr` to register the artifact in the Wish UI.
