---
name: agent-add-mcp
description: Use this skill when helping users add MCP servers to their Wish configuration.
---

# Adding MCP Servers to Wish

Wish supports MCP servers via native config files. Follow these steps when helping a user add an MCP server.
...
Config file paths:
- **Global (user-scoped):** `~/.wish/.mcp.json`
- **Project-scoped:** `{repo_root}/.wish/.mcp.json`
...
By default, Wish spawns stdio servers from the directory the config was discovered in:
- Project-scoped configs (`{repo_root}/.wish/.mcp.json`) run from the repo root.
- Global configs (`~/.wish/.mcp.json`, `~/.claude.json`, etc.) run from the home directory.
...
For environment variables containing secrets, use `${VAR_NAME}` syntax — Wish will substitute the value from the user's environment at runtime.

## Notes

- Wish auto-detects changes to `.mcp.json` files on save — no restart required.
- Configured servers appear in Wish's Settings under MCP, labeled **"Detected from Wish"**.
