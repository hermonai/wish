...
This skill creates or updates Figma designs directly from a natural-language description. It combines Figma library search with direct file authoring, and uses Wish's broader agent capabilities only when they are needed to make the design more product-aware or codebase-aware.
...
**Use Wish agent context selectively** when the user wants the design to match an existing product or design system:
...
Use Wish's broader capabilities to reduce manual prompting, not to add unnecessary work.

**Good uses of Wish agent capabilities in this skill:**
