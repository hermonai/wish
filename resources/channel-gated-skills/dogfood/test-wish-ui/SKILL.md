...
name: test-wish-ui
description: >
  Guides testing Wish UI features and changes using the computer use tool.
  Use this skill only when the computer_use tool is available to the agent.
  Covers launching Wish and verifying UI behavior.
user-invocable: false
---

# Computer Use for Wish UI Testing

Use the `computer_use` tool to visually test that Wish looks and behaves as intended after UI changes.

## Running Wish

Launch Wish from the repository root with:

```bash
cargo run -- --api-key $STAGING_USER_WISH_API_KEY
```

The `--api-key` flag authenticates using the API key from the `STAGING_USER_WISH_API_KEY` environment variable, so the app starts directly without interactive login prompts.
...
- The command to build and launch Wish (typically `cargo run -- --api-key $STAGING_USER_WISH_API_KEY` from the repo root)
