...
description: >
  ...
  Verifies changes by spawning a cloud agent with computer use to test Wish.
  Triggers a cloud agent that runs the test-wish-ui skill.
user-invocable: true
---
...
Verify the remote URL contains `hermonai/wish`. If it does not, warn the user that this skill only supports the wish repository and stop.

The environment ID for the Wish Dev Environment is `SVhg783GBFQHk1OfdPfFU9`.
...
- `base_prompt`: include an instruction to read and follow the `test-wish-ui` skill, followed by the verification task (see the next section)
...
The `test-wish-ui` skill is bundled, so the cloud agent has it automatically. Tell the agent to invoke it by name in the `base_prompt` (e.g. "Read and follow the test-wish-ui skill.").

### 4. Write an Effective Prompt

The prompt should tell the cloud agent:
- Which element, flow, or behavior to test
- What hardcoding or mocking is needed (see below and the test-wish-ui skill for details on sandbox constraints)
...
```
I added a migration that symlinks config from ~/.wish into ~/.wish-preview on first launch.
...
with test files. After launching Wish, verify the symlinks were created in ~/.wish-preview.
```

### Hardcoding to reach the code path under test

The cloud agent builds Wish with `cargo run`, which may not match the exact runtime conditions of your change (e.g., different channel, missing feature flags, absent preconditions). When this happens, instruct the cloud agent to temporarily hardcode the code so the build exercises the path you need to test. Common examples:
...
No extra surfacing step is needed — the Wish client displays the cloud agent run automatically.
