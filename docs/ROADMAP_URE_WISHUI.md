...
| **4** | **Tensor heatmaps in the standalone eframe viewer + `wish-world render tensor` CLI demo** | `wish-render`, `wish-world-cli` | `paint_tensor_overlay` matches the wishui pane visually. New CLI subcommand builds a five-tensor canvas (eye / linspace / ripple / Gaussian / sine-stack) and opens the native viewer end-to-end. | ✅ shipped |
| 5 | URE canvas pane (proper PaneContent) | `wish-canvas`, `wish-app` | New `PaneContent::UreCanvas` that hosts a `Canvas` in a Wish pane (not just an embedded element). |   |
| 6 | 3D toggle on canvas pane | `wish-render`, `wish-canvas`, `wish-app` | `View → 3D` projects canvas nodes via `scene3d::Camera3D`. Force-directed layout in 3D. |   |
| 7 | Tensor view pane | new `wish-tensor-view`, `wish-app` | `PaneContent::Tensor` — 1D line, 2D heatmap, 3D voxel slices over a `TensorRef`. |   |
| 8 | Wish Projects pane | `wish-app` (per `DESIGN_PROJECTS_PANE.md`) | `PaneContent::Projects`, runs SDLC into adjacent terminal panes. |   |
| 9 | Native Architecture renderer | `wish-app` over `wish-render` | `PaneContent::Architecture` — 3D directory cubes / dep graph. |   |
| 10 | Legacy-agent → Hermon MVP | `wish-app` (per `DESIGN_LEGACY_AGENT_HERMON_ROUTING.md`) | `AmQuerySuggestions` proxied through Hermon, behind a feature flag. |   |
| 10+ | Full legacy coverage, time-series tensors, tensor diff | TBD | Stretch — only if 1–10 are stable. |   |
