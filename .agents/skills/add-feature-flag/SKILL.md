...
description: Add a new feature flag to gate code changes in the Wish codebase.
---

# add-feature-flag

Add a new feature flag to gate code changes in the Wish codebase.

## Overview

Feature flags in Wish are compile-time flags that allow features to be selectively enabled for different channels (e.g.: Dev, Stable). They use a small runtime plumbing layer that checks if a flag is enabled.
...
When ready to enable the feature for all Wish Stable users, add it to the `default` array in `app/Cargo.toml`:
