...
description: Promote a feature-flagged feature to Dogfood, Preview, or Stable in the Wish codebase. Use when a feature behind a FeatureFlag is ready to roll out to a broader audience, including wiring up the compile-time/runtime bridge and deferring flag cleanup safely.
---
...
- **Runtime** (`crates/wish_features/src/lib.rs`): `DOGFOOD_FLAGS`, `PREVIEW_FLAGS`, `RELEASE_FLAGS` — enabled per-channel at startup.
...
Add the flag to `DOGFOOD_FLAGS` in `crates/wish_features/src/lib.rs`:
...
1. Add to `PREVIEW_FLAGS` in `crates/wish_features/src/lib.rs`.
...
Prefer this over adding to `RELEASE_FLAGS` (see comment at `crates/wish_features/src/lib.rs:787-790`). It compiles the feature into all builds and enables a one-line rollback.
...
### 3. `crates/wish_features/src/lib.rs` — remove from `PREVIEW_FLAGS` / `DOGFOOD_FLAGS`
