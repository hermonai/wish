...
description: Write, improve, and run Rust unit tests in the Wish Rust codebase.
---

# Rust Unit Tests in Wish
...
- Prefer `wishui::App::test` for deterministic unit tests around views/models.
- Initialize app models once, then mutate via `update` and assert via `read`.

```rust
use wishui::App;
// In app crate tests prefer `crate::test_util::...`; from other crates use `wish::test_util::...`.
...
```rust
use wish::features::FeatureFlag; // or `use crate::features::FeatureFlag;` inside the app crate
