...
description: Repo-specific dedupe guidance for the Wish repository. Only the categories declared overridable by the core dedupe-issue skill may be specialized here.
---

# Repo-specific dedupe guidance for `wish`
...
- Strip low-signal title prefixes such as `Bug:`, `Feature:`, `Request:`, `[Bug]`, `[Feature]`, `Wish:`, and platform tags like `[macOS]`, `[Linux]`, or `[Windows]` before comparing titles.
...
- Do not collapse distinct Wish surfaces just because they share a word like "agent", "terminal", "MCP", "settings", "search", or "sync". Require overlap in the actual failing behavior or requested capability.
