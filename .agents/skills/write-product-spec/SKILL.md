...
"User" is not limited to the end user of the Wish app. It means whoever consumes the surface being designed:

- For UI / UX features: the human using Wish.
...
14. Sharing or exporting a block (Wish Drive, share link, save as file) preserves the original markdown source, not the rendered form.

15. Theming: table borders, header backgrounds, alternating row shading (if any), and link/code styles all come from the active Wish theme. No hard-coded colors.
