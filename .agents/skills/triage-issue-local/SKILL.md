...
description: Repo-specific triage guidance for wish. Only the categories declared overridable by the core triage-issue skill may be specialized here.
---

# Repo-specific triage guidance for `wish`
...
- `wish` is the public-facing Wish desktop client repository. Treat public issue reports as potentially incomplete and avoid asking for secrets, tokens, private workspace names, private repository names, or account identifiers in the public issue thread.
- Distinguish the user's observed Wish behavior from their guesses about Rust modules, UI components, server behavior, feature flags, or product intent.
- For issue reports that mention another terminal, editor, shell, or CLI tool, identify whether the problem is Wish-specific or generally reproducible outside Wish before assigning Wish ownership.
...
- `area:wish-drive` for Wish Drive objects, sync, sharing, workflows, notebooks, tab configs, and persisted artifacts.
...
- Wish channel and version/build number, especially whether the report is for Dev, Canary, Preview, Beta, or Stable.
...
- Clear reproduction steps, expected behavior, actual behavior, frequency, regression timing, and whether the user can reproduce outside Wish.
...
- Performance issue: ask for approximate scale, how long Wish has been running, what action triggers the spike or hang, and whether logs or a profile are available.
