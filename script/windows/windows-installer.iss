...
#define MyAppPublisher "Hermon AI"
#define MyAppURL "https://wish.hermon.ai/"
#ifndef MyAppName
  #define MyAppName "WishDev"
#endif
#ifndef MyAppVersion
...
#define ChannelPascalCase \
  ...
  "Unknown")))))
#define AppMutexName "Local\Wish" + ChannelPascalCase + "_SingleInstance"


[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId=wish-terminal-{#ReleaseChannel}
AppName={#MyAppName}
...
; before running this installer. Therefore, we are good to force close Wish without fear of losing
; unsaved work.
...
; For manual installs: if Wish is running, show a dialog prompting the user to close it
...
[Registry]
Root: HKCU; Subkey: "SOFTWARE\Hermon AI\Wish\{#MyAppName}"; Flags: uninsdeletekey
; cleanup "Open Wish Here" registry entries
...
; Add "Open Wish in new tab" to directory context menu
...
; Add "Open Wish in new tab" to directory background context menu
...
; Add "Open Wish in new window" to directory context menu
...
; Add "Open Wish in new window" to directory background context menu
...
[Tasks]
Name: addToPath; Description: "Add Wish to PATH"
...
[Icons]
Name: "{autoprograms}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "ai.hermon.{#MyAppName}"
Name: "{autodesktop}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "ai.hermon.{#MyAppName}"; Tasks: desktopicon
...
{ Returns true when the installer was launched by Wish's auto-update code.
  The auto-update path passes /update=1 on the command line and /NOCLOSEAPPLICATIONS
  so that the installer does not forcibly kill the running Wish process. Instead we
  wait for Wish to exit naturally by polling the app mutex below. }
...
{ For background updates, return an empty mutex name so that Inno Setup skips its
  ...
  installs, return the real mutex name so the user is prompted to close Wish first. }
...
var
  ...
  ResultCode: Integer;
begin
  { Background update: the installer was launched while Wish was still running.
    We passed /NOCLOSEAPPLICATIONS so Inno won't kill it. Wait here - before any
    files are touched - for Wish to release its single-instance mutex, which
    happens as part of normal process exit. }
  if CurStep = ssInstall then
  begin
    if IsBackgroundUpdate() then
    begin
      Log('Background update: waiting for Wish to exit (mutex: {#AppMutexName})...');
      WaitCounter := 0;
      ...
      begin
        Log('Wish mutex still held after timeout; force-killing remaining processes.');
        { Kill by image name. {#MyAppExeName} (e.g. warp.exe, dev.exe) is unique
          ...
          rely on Wish's Job Object (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) to
        ...
        Log('Wish has exited; proceeding with file installation.');
    end;
  end;

  { After a successful install, write a helper script for running the Wish CLI. }
  { We use this to add a "warp-" prefix (e.g. "warp-preview.cmd" vs. "preview.exe") }
  if CurStep = ssPostInstall then begin
    { Add Wish to PATH if requested }
