...
- `crates/wish_files/src/lib.rs (95-106)` — `FileBackend::Remote`; already supports remote save/delete
...
- **Regression**: Run existing `diff_application_tests.rs` and `cargo nextest run -p wish_files` to verify local path is unchanged.
