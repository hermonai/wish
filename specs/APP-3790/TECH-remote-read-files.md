...
The remote server already runs on the host machine with full filesystem access and has access to the same dependencies (`wish_files`, `wish_util`, `mime_guess`). Rather than building a degraded client-side approximation, we push the file-reading logic to the server so the ReadFiles tool has full feature parity with local: line-range extraction, binary/image support, metadata, and size limits.
...
- `crates/wish_files/src/lib.rs:528` — `FileModel::read_text_file`; line-range extraction and byte-limit truncation
- `crates/wish_util/src/file_type.rs:46` — `is_binary_file`; extension-based binary detection
...
4. Binary detection via `is_binary_file` (extension-based, `wish_util::file_type`)
