...
## Context
The product spec describes a chip + `/move-to-cloud` slash command that opens a split cloud-mode pane next to the local agent to hand off the active local Hermon Agent conversation to the cloud. The user types the follow-up prompt and submits inside the pane's existing cloud-mode input bar; the cloud agent runs in a fresh sandbox, gets a forked copy of the conversation history, and rehydrates from a workspace snapshot taken on the local machine.
...
sequenceDiagram
    ...
    Wk->>Sand: hermon agent run --task-id <new_task_id> --sandboxed (no --conversation flag)
    ...
    Sand->>Sand: driver_options.resume = ResumeOptions::Hermon(Historical{...})
...
`DispatchedAgent` (`app/src/terminal/view/ambient_agent/view_impl.rs`) renders the user's prompt via `insert_cloud_mode_queued_user_query_block` (REMOTE-1454's helper) when `is_local_to_cloud_handoff()` is true. The block is removed on the same transitions the non-hermon harness path already handles in `handle_ambient_agent_event`: `Failed`, `Cancelled`, `NeedsGithubAuth`, `HarnessCommandStarted`. For the Hermon handoff specifically, the first `AppendedExchange` also clears the block (the analogous "harness CLI started" transition for Hermon). Each path calls `remove_pending_user_query_block(ctx)` (idempotent). The cloud agent's exchanges flow into the pane via the shared-session replication path that regular cloud-mode runs already use.
...
1. **Worker.** `hermon-agent-worker/internal/common/task_utils.go::AugmentArgsForTask` passes only `--task-id <T>` (never `--conversation`). Pinned by `task_utils_test.go:152` ("does not forward --conversation even when AgentConversationID is set").
...
3. **Resume.** `load_conversation_information(<forked_id>, HarnessKind::Hermon)` (`mod.rs:1105`) calls `get_ai_conversation(<forked_id>)` and produces `ResumeOptions::Hermon(ConversationRestorationInNewPaneType::Historical { conversation, ... })`. The terminal driver restores the conversation and the agent starts with the forked history visible.
...
### 10. Feature flags
Add `FeatureFlag::HandoffLocalCloud` in `crates/wish_features/src/lib.rs`. The chip, slash command, client API methods, and server endpoint behavior are all gated on `HermonHandoff && HandoffLocalCloud`. Both flags must be enabled for the feature to function.
...
## Follow-ups
- Extend `/move-to-cloud` to dispatch to non-Hermon harnesses (Claude Code, Gemini, etc.). Most of the plumbing (touched-repo derivation, snapshot pipeline, upload-snapshot endpoint, server-side fork) is reusable; the differences are (a) the chip/command gating drops the Hermon-only check and instead reads the active conversation's `harness_kind()` to pick the cloud-side resume strategy, (b) for Claude conversations the server handler must also upload the local Claude transcript envelope to the right GCS slot so the cloud Claude run resumes via REMOTE-1373's existing transcript rehydration path.
- A CLI surface for handoff (e.g. `hermon agent handoff --conversation <local-id> --env <id> --prompt "..."`). Opens up automation. Out of scope for V0; the public API surface is already CLI-friendly when we get there.
