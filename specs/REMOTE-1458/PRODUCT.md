...
- `app/src/terminal/view/pane_impl.rs` renders a single plain `HermonCloud` glyph for every ambient run via `render_ambient_agent_indicator` and an unbadged Oz/HermonCloud glyph for conversation-bound terminals via `render_agent_indicator`. No brand circle, no status badge, no harness distinction, no local-vs-cloud distinction.
...
- Spawn an Hermon Cloud run and confirm the three surfaces show Oz purple + cloud lobe + status, matching each other.
...
- Open the inline conversation list menu after running several agents (Hermon Cloud, Claude cloud, Gemini cloud, local Claude, local Oz) and confirm each row's leading icon correctly identifies the agent and ambient-ness.
