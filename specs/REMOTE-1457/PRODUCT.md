...
3. The dropdown exposes exactly these options, in this order:
    ...
    2. `Warp Agent` — the default/Hermon harness.
...
19. Every run card in the list renders a `Harness: <display name>` segment on its metadata line, immediately after the `Source: …` segment and before `Run time: …` / `Credits used: …`, using the existing ` • ` separator between segments. Example: `Source: Hermon Web • Harness: Claude Code • Run time: 2 minutes`.
