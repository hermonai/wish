...
The server is adding `POST /harness-support/notify-user` and `POST /harness-support/finish-task` endpoints (see `warp-server` spec). Third-party harnesses invoke server APIs through the `hermon` CLI, not directly. We need:

1. Two new `hermon harness-support` subcommands that call these endpoints.
...
- `../claude-code-warp-internal/plugins/hermon-harness-support/` — existing plugin (skills, hooks)

## Current State

`hermon harness-support` has two subcommands: `ping` and `report-artifact`. Each follows the same pattern:
...
The Claude Code plugin currently has three skills (`hermon-report-pr`, `hermon-report-artifact`, `hermon-report-plan`) and a hook that auto-reports plans. Skills are either shell scripts calling `$HERMON_CLI harness-support ...` or SKILL.md instructions.
...
### 5. Claude Code plugin (`../claude-code-warp-internal/plugins/hermon-harness-support/`)

**New skill: `hermon-notify-user`**

`skills/hermon-notify-user/SKILL.md` — instructs Claude Code to call:
```sh
$HERMON_CLI harness-support notify-user --message '<message>'
```
when it wants to send a progress update to the user (e.g., after completing a milestone).

**New skill: `hermon-finish-task`**

`skills/hermon-finish-task/SKILL.md` — instructs Claude Code to call a shell script:
```sh
$HERMON_CLI harness-support finish-task --status <success|failure> --summary '<summary>'
```
...
sequenceDiagram
    ...
    participant HERMON as hermon CLI
    participant SA as ServerApi (client)
    participant WS as warp-server

    Note over CC,WS: Progress update
    CC->>HERMON: hermon harness-support notify-user --message "..."
    HERMON->>SA: HarnessSupportClient::notify_user("...")
    ...
    CC->>HERMON: hermon harness-support finish-task --status success --summary "..."
    HERMON->>SA: HarnessSupportClient::finish_task(true, "...")
...
- **Plugin skills**: test shell scripts with mock `HERMON_CLI` to verify correct arg construction.
