...
## Relevant code
- `crates/wish_core/src/paths.rs` — owns app data/config paths and the new Warp home config directory helpers.
...
## Path helper design
Add purpose-specific helpers in `wish_core::paths`:
...
Initial Warp skill loading reads from `warp_managed_skill_dirs()`, which resolves to `wish_core::paths::warp_home_skills_dir()` when home exists.
...
The resolver fallback scans home provider paths in provider precedence order. For `SkillProvider::Warp`, `home_skills_path(SkillProvider::Warp)` resolves to `wish_core::paths::warp_home_skills_dir()`.
...
```
The config path is `wish_core::paths::warp_home_mcp_config_file_path()`. The logical root path remains `dirs::home_dir()` so `FileBasedMCPManager` treats it as user-scoped MCP config.
## MCP path classification
`home_config_file_path(MCPProvider::Warp)` returns `wish_core::paths::warp_home_mcp_config_file_path()`.
...
- Add or update `wish_core::paths` tests for Warp home config directory, Skills directory, and MCP config path helpers.
...
- Run targeted commands such as:
  - `cargo test -p wish_core --lib paths`
