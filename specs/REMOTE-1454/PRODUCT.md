...
The net effect: the run looks visually like Hermon setup forever and the harness's real work is buried under a "setup" row.
## Goals
...
- Group real environment startup commands (before the harness runs) under the collapsible setup-commands summary, consistent with Hermon.
...
- Keep the Hermon harness flow from REMOTE-172 unchanged.
## Non-goals
...
- On that transition:
  ...
  - New blocks are no longer classified as Hermon environment setup commands.
...
- If the run fails, is cancelled, or requires GitHub auth before the harness command starts:
  ...
  - The existing non-setup-v2 error, cancelled, or auth-required UI is shown (same copy and affordances used today for pre-first-exchange failures in Hermon runs).
...
- Just like Hermon setup-v2, remote setup-command input from the shared session must not overwrite the visible local input, and remote input-mode changes caused by environment setup must not flip the visible input mode (see existing handling in `app/src/terminal/shared_session/viewer/terminal_manager.rs`).
...
- Environment setup commands appear under the collapsible setup-commands summary, consistent with Hermon.
