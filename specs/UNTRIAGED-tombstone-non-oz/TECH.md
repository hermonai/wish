...
Separately, both the tombstone and `ConversationDetailsPanel` unconditionally surface the `Continue locally` button on non-transcript runs. `ContinueConversationLocally` only forks Hermon-harness conversations, so for non-Hermon tasks the button is dead UI.
...
- `crates/wish_cli/src/agent.rs` — `Harness` enum (`Hermon`, `Claude`, `Gemini`).
...
`TombstoneDisplayData` gains one field: `harness: Option<Harness>`, defaulting to `None`. `enrich_from_task` sets it from `task.agent_config_snapshot`: snapshot present + explicit harness → `Some(harness)`; snapshot present + no explicit harness → `Some(Hermon)` (default); snapshot absent → stays `None`.

`render_action_buttons` (cfg `not(wasm)`) and `ConversationDetailsPanel::continue_locally_conversation_id` both apply the same gate: `!matches!(harness, Some(h) if h != Harness::Hermon)`. `None` and `Some(Hermon)` both show the button; only confirmed non-Hermon hides it. No `is_task` flag is needed — plain conversations and pre-load tasks naturally fall into the `None` bucket.
...
Notes:
- All three metadata fields are unguarded: when the task has a value, it wins. When it doesn't, whatever `from_conversation` populated (or default-empty for non-Hermon) survives.
- Local Hermon runs have no `task_id`, so `enrich_from_task` never runs and `from_conversation` remains the only source. No regression there.
...
- The harness gate is permissive on `None`: cloud non-Hermon tasks briefly show the button until the fetch resolves and switches `harness` to `Some(non-Hermon)`.
...
- **`Continue locally` flashes briefly on non-Hermon tombstones** before the task fetch resolves. Conscious trade-off: hiding eagerly would also hide the button on local conversations and cloud Hermon runs (we don't know the harness yet), which is worse.
- **Snapshot missing harness on a non-Hermon run** mis-shows `Continue locally` because `Some(snapshot) + harness == None` defaults to `Some(Hermon)`. In practice the server always populates `harness` when a non-Hermon run is dispatched; if that ever changes the gate becomes too permissive.
...
- **Partial task data**: if `run_time()` or `credits_used()` is `None`, we leave whatever `from_conversation` set (avoids blanking Hermon when the task is partially populated).

## Tests
Unit tests next to `conversation_ended_tombstone_view.rs`:
1. Hermon with task: run time / credits overwritten by task values.
2. Hermon with task missing those fields: `from_conversation` values preserved.
3. Non-Hermon happy path: empty defaults populated from a fully-populated task.
...
8. Snapshot without explicit harness defaults to `Some(Harness::Hermon)`.
...
Manual: cloud Claude Code (`Continue locally` hidden in both surfaces once the task loads), cloud Hermon (numbers should match the details panel; `Continue locally` shown), local Hermon (unchanged). Run `./script/presubmit` before PR.

## Follow-ups
- Wasm parity (fetch the task and run `enrich_from_task` on web; share with the panel's wasm path).
- `Directory:` / `Branch:` for non-Hermon via a future `POST /harness-support/report-context` sibling endpoint piggy-backed under `try_join!` with the existing uploads.
- Shared helper translating `AmbientAgentTask` → display strings if a third surface ever needs it.
- If/when `ContinueConversationLocally` learns to fork non-Hermon runs, drop the harness gate (or invert it to whatever the new constraint is).
