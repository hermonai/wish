...
- `app/src/workspace/view/left_panel.rs:1213` — `deduplicate_by_directory_name`
  ...
  happen upstream).
- `crates/wish_util/src/path.rs` + `path_test.rs` — target location for the
  shared `group_roots_by_common_ancestor` helper.
- `crates/wish_util/src/standardized_path.rs` — `StandardizedPath` impls
...
### 1. Shared helper in `wish_util::path`

Add a generic ancestor-dedup helper usable by both views.

```rust path=null start=null
// crates/wish_util/src/path.rs
...
Place unit tests next to the impl in `crates/wish_util/src/path_test.rs`
...
fn group_std_roots_by_common_ancestor(
    ...
    wish_util::path::group_roots_by_common_ancestor(roots)
}
```
...
pub fn set_root_directories(&mut self, roots: Vec<PathBuf>, _ctx: &mut ViewContext<Self>) {
    let grouping = wish_util::path::group_roots_by_common_ancestor(&roots);
...
sequenceDiagram
    ...
    participant U as wish_util::path
...
### Unit tests (`crates/wish_util/src/path_test.rs`)
...
`wish_util::path::group_roots_by_common_ancestor`, their surviving-root
