...
- `wish_util/src/worktree_names.rs` — pure naming module (word list, `generate_name`, `generate_unique_name`, `generate_worktree_branch_name`)
...
### 1. Pure naming module in `wish_util`

New file: `wish_util/src/worktree_names.rs`. This module has zero I/O — it takes a set of existing branch names and returns a unique name. Keeping it in `wish_util` isolates it from the `app` crate's compile graph and keeps it trivially testable.

`rand` needs to be added as a dependency of `wish_util` (it's already a workspace dep).
...
Add `pub mod worktree_names;` to `wish_util/src/lib.rs`.
...
- **Unit tests in `wish_util/src/worktree_names.rs`**:
