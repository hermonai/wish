...
2. The command appears in the slash-command menu (and is reachable via keybinding) only when the active conversation is associated with a cloud Oz `AmbientAgentTask`. For local conversations and for non-Hermon Cloud runs (Claude, Gemini), the command is hidden, matching the harness gate already used by the existing button entrypoints.
