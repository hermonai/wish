# Product Spec: `--jq` filter flag for `hermon run get` and `hermon run list`
...
Add a `--jq "<filter>"` flag to `hermon run get` and `hermon run list`. When set, the command runs the server's JSON response through the given jq filter (using the `jaq` Rust implementation) and writes the filter's result to stdout instead of printing the raw response.
...
```
hermon run list --state failed --output-format json | jq -r '.runs[].task_id'
```
...
1. Both `hermon run list` and `hermon run get` accept an optional `--jq <FILTER>` flag. `<FILTER>` is a jq filter expression in the dialect implemented by [`jaq`](https://github.com/01mf02/jaq) with the full standard library enabled (via `jaq-all`). When not set, both commands behave exactly as they do today.

2. When `--jq` is set, the input to the filter is the same JSON document the command would emit under `--output-format json`:
   - For `hermon run list`, the full list response envelope, e.g. `{ "runs": [...], "page_info": { ... } }`.
   - For `hermon run get <run_id>`, the single run object returned by `GET /api/v1/agent/runs/:runId`.
...
4. Output format of the filter's result:
   ...
   - **Scalar values are printed as raw text**, not JSON-encoded. Specifically: strings are printed without surrounding double quotes and without JSON escapes; numbers are printed in their natural form; `true`, `false`, and `null` are printed as the literal words `true`, `false`, and `null`. This matches `gh --jq`'s behavior ([cli/cli#3012](https://github.com/cli/cli/pull/3012)) and means `hermon run list --jq '.runs[].task_id'` prints one bare task ID per line, not one JSON-quoted string per line.
...
7. `--jq` does not change server-side filtering behavior. Existing `hermon run list` filter flags (`--state`, `--source`, etc.) compose with `--jq`: the server narrows the response first, and the jq filter runs on the narrowed response.

8. `--jq` is documented in `hermon run list --help` and `hermon run get --help`. The help text:
...
9. Invariants that must not regress:
   - `hermon run list` and `hermon run get` with no `--jq` produce byte-identical output to today for each `--output-format`.
...
- No `--jq` on other commands (e.g. `hermon run conversation get`, `hermon environment list`). The implementation is structured so that adding `--jq` to any command that already uses `print_raw_json` is a small, mechanical follow-up, but those rollouts are out of scope here.
...
hermon run list --state failed --updated-after 2026-04-20T00:00:00Z \
  --jq '.runs[].task_id'

# Get a single field from a run.
hermon run get 01HX... --jq '.state'

# Compose with other flags; --jq runs after server-side filters.
hermon run list --source CLI --limit 100 \
