...
Feature flags are defined in `crates/wish_features/src/lib.rs`. `CloudModeSetupV2` already exists near the end of the enum at `crates/wish_features/src/lib.rs:827`, and the rollout arrays are defined at `crates/wish_features/src/lib.rs:852`, `crates/wish_features/src/lib.rs:910`, and `crates/wish_features/src/lib.rs:926`. There is currently no `HandoffCloudCloud` flag. The Cargo feature graph in `app/Cargo.toml` is the right place to encode that `handoff_cloud_cloud` depends on `cloud_mode_setup_v2`.
...
Add `HandoffCloudCloud` to `FeatureFlag` in `crates/wish_features/src/lib.rs`, preserving the enum's chronological ordering. Keep the description concise and product-focused, for example gating cloud-to-cloud continuation of cloud mode conversations.
