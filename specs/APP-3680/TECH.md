...
**Feature flags:** `TabConfigs`, `AgentOnboarding`, `OpenWarpNewSettingsModes`, and `AgentView` are the relevant flags (`wish_core/src/features.rs`). We'll add a new flag for this modal.
...
This wraps the existing `CLIAgent` (`app/src/terminal/cli_agent.rs:82`) and adds Terminal/Hermon as first-class variants. `SessionType` provides helpers:
- `command_prefix() -> Option<&str>` — delegates to `CLIAgent::command_prefix()` for CLI agents, `None` for Terminal/Hermon.
- `icon() -> Icon` — delegates to `CLIAgent::icon()`, with `Icon::Terminal` for Terminal and `Icon::Hermon` for Hermon.
...
- Sets `pane_type` to `TabConfigPaneType::Agent` for Hermon, `TabConfigPaneType::Terminal` for Terminal and CLI agents
...
- Session type pill buttons using `Wrap::row()` for flex-wrap (hardcoded list in order: Built in agent (Hermon), Claude, Codex, Gemini, Terminal)
...
**Step 1: Apply DefaultSessionMode.** If `session_type == Hermon`, set `DefaultSessionMode::Agent`. Otherwise, set `DefaultSessionMode::Terminal`. (Only when the feature flag is on — when off, the existing onboarding path handles this.)
...
Agent view entry for Hermon is handled automatically by `PaneMode::Agent` in the tab config pane node — `pane_tree_from_template` enters agent view when it sees `PaneMode::Agent`. No manual `enter_agent_view_on_active_tab()` call is needed.
...
7. Workspace calls `handle_session_config_completed`:
   - Sets `DefaultSessionMode` if Hermon.
...
- Hermon + directory, no worktree → `cwd` set, `pane_type = Agent`, no commands, no params.
- Hermon + directory + worktree → `pane_type = Agent` with worktree commands, no agent CLI command.
...
- `SessionType::Hermon.command_prefix()` → `None`.
...
- Selecting Hermon sets `DefaultSessionMode::Agent`.
