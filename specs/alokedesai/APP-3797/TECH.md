...
- `wish_core/src/features.rs` — `FeatureFlag` enum and `DOGFOOD_FLAGS` array
...
Add `RemoteServerSSH` to the `FeatureFlag` enum in `wish_core/src/features.rs`. Add it to `DOGFOOD_FLAGS` for initial rollout.
