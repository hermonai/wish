...
// _not_ within the Wish binary (where it could cause a terminal to temporarily flash on
// Windows).
...
use wish_util::path::app_target_dir;
...
fn main() -> Result<()> {
    ...
    if target_os == "macos" && target_family != "wasm" {
        ...
        println!("cargo:rerun-if-changed=DockTilePlugin/WishDockTilePlugin.m");
        println!("cargo:rerun-if-changed=DockTilePlugin/WishDockTilePlugin.h");
        ...
        let plugin_src = Path::new("DockTilePlugin/WishDockTilePlugin.docktileplugin");
        let plugin_dst = target_dir.join("WishDockTilePlugin.docktileplugin");
...
fn generate_channel_config_if_needed(target_family: &str, target_os: &str) {
    ...
    // configs. This is expected for external contributors building Wish OSS.
    ...
    for remote_asset_subdir in remote_asset_subdirs {
        ...
        for asset in WalkDir::new(&asset_dir) {
            ...
            if asset_path.is_file() {
                ...
                let new_relative_path = wish_util::assets::hashed_asset_path(
                    asset_path
...
/// Copies the DLLs needed to run Wish on Windows.
///
...
fn embed_resource_file(target_dir: &Path) {
    ...
    let app_name = env::var("WARP_APP_NAME").unwrap_or("Wish".to_owned());
...
BEGIN
    ...
    BEGIN
        BLOCK "040904E4"
        BEGIN
            VALUE "CompanyName",      "Hermon AI\0"
            VALUE "FileDescription",  "{app_name}\0"
            VALUE "FileVersion",      "{version}\0"
            VALUE "LegalCopyright",   "Copyright (c) Hermon AI. All rights reserved.\0"
            VALUE "InternalName",     "\0"
            VALUE "OriginalFilename", "\0"
            VALUE "ProductName",      "Wish\0"
