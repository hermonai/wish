...
use wish_core::{settings::Setting, ui::appearance::Appearance};
...
impl SettingsImportView {
    ...
    fn render_reset_button(&self, appearance: &Appearance) -> Box<dyn wishui::Element> {
        appearance
            .ui_builder()
            ...
            .with_centered_text_label("Reset to Wish defaults".to_owned())
            .build()
            .on_click(move |ctx, _, _| {
                            ...
                            .with_child(
                                Flex::row()
                                    ...
                                    .with_cross_axis_alignment(
                                        wishui::elements::CrossAxisAlignment::Center,
                                    )
                                    .with_main_axis_size(MainAxisSize::Max)
    ...
    pub(crate) fn interrupt_block(&mut self, ctx: &mut wishui::ViewContext<Self>) {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            SettingsImportAction::ImportButtonClicked => {
                ...
                // set_preferences should not fail because it is writing directly to Wish's preferences.
