...
use wishui::text_layout::TextFrame;
use wishui::windowing::WindowManager;
use wishui::{AddSingletonModel, App, UpdateModel, UpdateView};
...
fn test_smart_select_with_drag() {
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let mut editor = EditorView::new_with_base_text(
                "word ~/.warp/themes/foo-bar.yaml andy@wish.dev",
                Default::default(),
                ctx,
            );
            editor.select_word(&DisplayPoint::new(0, 8), ctx);
...
fn test_autosuggestions() -> Result<()> {
    use wishui::text_layout::LayoutCache;
...
fn test_paste_clipboard_with_text_only_should_paste_text_normally() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let clipboard_content = wishui::clipboard::ClipboardContent {
...
fn test_paste_clipboard_with_image_only_should_switch_to_agent_mode() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let png_image = wishui::clipboard::ImageData {
...
fn test_paste_clipboard_with_unsupported_image_and_text_should_show_error() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let bmp_image = wishui::clipboard::ImageData {
