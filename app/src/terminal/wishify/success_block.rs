...
pub struct WishifySuccessBlock {
    ...
    auto_wishify_snippet: Option<AutoWishifySnippet>,
}
...
impl WishifySuccessBlock {
    ...
    pub fn new(
        ...
        let is_auto_wishify_configured = subshell_info
            .as_ref()
            .map(|info| info.was_triggered_by_rc_file_snippet)
            .unwrap_or_default();

        let auto_wishify_snippet = if is_auto_wishify_configured {
            None
        } else {
        ...
        let auto_wishify_snippet = auto_wishify_snippet.map(|(output_grid, can_write_to_rc)| {
            AutoWishifySnippet {
        ...
        Self {
            ...
            spawning_command,
            auto_wishify_snippet,
        }
    }

    pub fn selected_text(&self) -> Option<String> {
        self.auto_wishify_snippet
            .as_ref()
    ...
    pub fn on_warpified_session_complete(&mut self, ctx: &mut ViewContext<Self>) {
        self.clear_auto_wishify_snippet(ctx);
    }

    pub fn clear_auto_wishify_snippet(&mut self, ctx: &mut ViewContext<Self>) {
        self.auto_wishify_snippet = None;
        ctx.notify();
    }
    ...
    pub fn render_output_grid(
        ...
        let auto_wishify_snippet = self.auto_wishify_snippet.as_ref()?;

        if auto_wishify_snippet.output_grid.is_empty() {
            return None;
        }

        let shell_language = ProgrammingLanguage::Shell(auto_wishify_snippet.shell_type);
        let runnable_command = render_runnable_code_snippet(
            &auto_wishify_snippet.output_grid,
            if auto_wishify_snippet.can_write_to_rc {
                Some(&shell_language)
            } else {
                None
            },
            ...
            Some(auto_wishify_snippet.code_snippet_handles.clone()),
            app,
        );

        let semantic_selection = SemanticSelection::as_ref(app);
        let selected_text = auto_wishify_snippet.selected_text.clone();
        ...
        let output_grid = SelectableArea::new(
            auto_wishify_snippet.selection_handle.clone(),
        ...
        let output_grid = Flex::column()
            .with_child(
                Container::new(
                    Text::new(
                        auto_wishify_snippet.description.clone(),
