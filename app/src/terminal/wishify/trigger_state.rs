...
use super::success_block::WishifySuccessBlock;
...
pub enum SshBlockState {
    Wishifying {
        handle: ViewHandle<SshWishifyBlock>,
    },
    WishifySuccess {
        handle: ViewHandle<WishifySuccessBlock>,
    },
    InstallTmux {
...
impl SshBlockState {
    ...
    pub fn get_block_view_id(&self) -> EntityId {
        match self {
            SshBlockState::Wishifying { handle, .. } => handle.id(),
            SshBlockState::WishifySuccess { handle, .. } => handle.id(),
    ...
    pub fn collapse_script(&mut self, ctx: &mut ViewContext<TerminalView>) -> bool {
        match self {
            ...
            SshBlockState::Wishifying { .. } => false,
            SshBlockState::WishifySuccess { .. } => false,
    ...
    pub fn focus(&mut self, ctx: &mut ViewContext<TerminalView>) {
        match self {
            SshBlockState::Wishifying { handle } => {
                handle.update(ctx, |block, ctx| block.focus(ctx));
            }
            SshBlockState::WishifySuccess { .. } => {}
            SshBlockState::InstallTmux { handle } => {
                handle.update(ctx, |block, ctx| block.focus(ctx));
            }
    ...
    pub fn get_system_details(&self, app: &AppContext) -> Option<SystemDetails> {
        match self {
            ...
            SshBlockState::Wishifying { .. } => None,
            SshBlockState::WishifySuccess { .. } => None,
    ...
    pub fn on_warpified_session_complete(
        ...
        match self {
            SshBlockState::InstallTmux { .. } | SshBlockState::Wishifying { .. } => {
                let block_id = self.get_block_view_id();
                return Some(block_id);
            }
            SshBlockState::WishifySuccess { handle } => {
...
/// Temporary state used to trigger Wishification.
#[derive(Default)]
struct WishifyTriggerState {
    block_id: Option<BlockId>,

    /// Lets us abort an attempt to auto warpify if the subshell command
    /// hasn't completed.
    auto_wishify_abort_handle: Option<SpawnedFutureHandle>,
    ...
    /// The command which may trigger ssh Wishification
    pending_command: Option<String>,
    /// The Host which may trigger ssh Wishification
    pending_wishify_ssh_host: Option<String>,

    /// Which, if any, SSH block is currently added to the blocklist.
    ssh_block_state: Option<SshBlockState>,

    ssh_wishify_timeout_handle: Option<SpawnedFutureHandle>,
...
#[derive(Default)]
pub struct WishifyState {
    session_id: Option<SessionId>,

    pending_state: Option<WishifyTriggerState>,
...
impl WishifyState {
    ...
    pub fn add_auto_wishify_abort_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.auto_wishify_abort_handle = Some(spawned_future_handle);
    }

    pub fn abort_auto_wishify(&mut self) {
        if let Some(abort_handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.auto_wishify_abort_handle.take())
        {
            abort_handle.abort();
        };
    }

    pub fn add_ssh_wishify_timeout_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.ssh_wishify_timeout_handle = Some(spawned_future_handle);
    }

    pub fn abort_ssh_wishify_timeout(&mut self) {
        self.replace_timeout_id();
        if let Some(handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.ssh_wishify_timeout_handle.take())
        {
            handle.abort();
        };
    }
    ...
    pub fn get_pending_ssh_host(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &WishifyTriggerState| state.pending_wishify_ssh_host.clone())
    }

    pub fn get_pending_ssh_command(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &WishifyTriggerState| state.pending_command.clone())
    }

    pub fn take_pending_ssh_host(&mut self) -> Option<String> {
        self.pending_state
            .as_mut()
            .and_then(|state: &mut WishifyTriggerState| state.pending_wishify_ssh_host.take())
    }

    pub fn clear_pending_ssh_host(&mut self) {
        if let Some(ref mut pending_state) = self.pending_state.as_mut() {
            pending_state.pending_wishify_ssh_host = None;
        }
    }
    ...
    pub fn set_pending_ssh_host(&mut self, command: String, ssh_host: Option<String>) {
        ...
        pending_state.pending_wishify_ssh_host = ssh_host;
    }
    ...
    /// and we have a Wishify Success block.
    fn on_warpified_session_complete(
        &mut self,
        state: WishifyTriggerState,
        ctx: &mut ViewContext<TerminalView>,
    ) -> Option<EntityId> {
    ...
    pub fn on_wishify_start(&mut self, active_session_id: Option<SessionId>) {
        self.session_id = active_session_id;
    }

    /// Called whenever a block is completed, to determine whether a Wishified session
    /// has been completed.
    pub fn get_completed_wishify_session_id(
        &mut self,
