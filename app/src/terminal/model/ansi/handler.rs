...
pub trait Handler {
    ...
    /// before it exits. Gives the Wish client a chance to drop per-session
    ...
    /// created. If the session is for a subshell, this should triggers Wish's bootstrap process.
    ...
    fn remote_warpification_is_unavailable(&mut self, _data: WishificationUnavailableReason) {}
    ...
    /// These allow external programs to trigger notifications in Wish.
