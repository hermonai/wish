...
/// after a `BlockFinished` hook but before Wish has written the next command from
...
impl EarlyOutput {
    ...
    fn handle_potential_typeahead(&mut self, ch: char) -> bool {
        ...
        if is_typeahead {
            ...
            if wish_core::channel::ChannelState::channel()
                == wish_core::channel::Channel::Integration
            {
                log::info!(
    ...
    pub fn preexec(block_list: &mut BlockList) {
        if block_list.early_output().mode == TypeaheadMode::ShellReported {
            ...
            // already written the command to the pty, resulting in Wish treating it as
            // background output.
