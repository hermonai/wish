...
use wishui::elements::ZIndex;
use wishui::event::ModifiersState;
use wishui::units::{IntoLines, IntoPixels, Pixels};
...
pub struct WaterfallGapElement {
    ...
    origin: Option<wishui::elements::Point>,
...
impl Element for WaterfallGapElement {
    ...
    fn after_layout(&mut self, ctx: &mut wishui::AfterLayoutContext, app: &AppContext) {
        self.input_element.after_layout(ctx, app);
        self.block_list_element.after_layout(ctx, app);
    }

    fn paint(&mut self, origin: Vector2F, ctx: &mut wishui::PaintContext, app: &AppContext) {
        self.origin = Some(wishui::elements::Point::from_vec2f(
            origin,
            ctx.scene.z_index(),
        ));
    ...
    fn origin(&self) -> Option<wishui::elements::Point> {
        self.origin
    }
    ...
    fn dispatch_event(
        ...
        if !handled {
            ...
            handled |= match event_at_z_index {
                Some(wishui::Event::LeftMouseDown { position, .. }) => {
                    self.mouse_down(*position, ctx)
                }
                Some(wishui::Event::RightMouseDown { position, .. }) => {
...
impl ScrollableElement for WaterfallGapElement {
    fn scroll_data(&self, app: &AppContext) -> Option<wishui::elements::ScrollData> {
