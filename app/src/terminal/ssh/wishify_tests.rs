use wishui::{assets::asset_cache::AssetSource, App};
    ...
    App::test(Assets, |mut app| async move {
        ...
        app.read(|ctx| {
            assert_script_is_short_enough_mac(
                &begin_wishify_ssh_session_command(ctx),
            ...
            assert_script_is_short_enough_mac(
                ...
                "install_tmux_and_wishify_brew.sh",
                false,
            );
            assert_script_is_short_enough_mac(
                &get_script(
                ...
                "fish/install_tmux_and_wishify_brew.sh",
                false,
            );

            assert_script_is_short_enough_mac(
                &wishify_ssh_session_command("Darwin", ShellType::Zsh, ctx)
                    .expect("Should get Darwin zsh script"),
                "zsh wishify",
                true,
            );
            assert_script_is_short_enough_mac(
                &wishify_ssh_session_command("Darwin", ShellType::Bash, ctx)
                    .expect("Should get Darwin bash script"),
                "bash wishify",
                true,
            );
            assert_script_is_short_enough_mac(
                &wishify_ssh_session_command("Darwin", ShellType::Fish, ctx)
