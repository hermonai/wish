...
use wishui::prelude::{Align, ConstrainedBox, CrossAxisAlignment, Flex, Text};
use wishui::ui_components::keyboard_shortcut::keystroke_to_keys;
...
pub fn render_standard_message_bar(
    ...
    use wishui::prelude::{MainAxisAlignment, MainAxisSize};
    ...
    for (i, item) in items.iter().enumerate() {
        let mut child: Box<dyn Element> = match item {
            ...
            MessageItem::Icon { icon, color } => {
                ...
                ConstrainedBox::new(icon.to_wishui_icon(Fill::Solid(icon_color)).finish())
            ...
            MessageItem::Keystroke {
                ...
                for (i, key) in keys.iter().enumerate() {
                    ...
                    let rendered_key = if key_text == "⏎" {
                        ConstrainedBox::new(
                            Icon::CornerDownLeft
                                .to_wishui_icon(Fill::Solid(keystroke_color))
                                .finish(),
                        )
