...
use crate::{
    ...
    util::openable_file_type::{is_file_openable_in_wish, OpenableFileType},
};
...
impl TerminalView {
    pub(super) fn maybe_suggest_open_in_wish(
        &mut self,
                ...
                move |view, maybe_match, ctx| {
                    if let Some(openable_path) = maybe_match {
                        if matches!(
                            ...
                            view.suggest_open_in_wish(openable_path, session, ctx);
                        }
                    }
                },
            );
    ...
    pub fn is_open_in_wish_banner_open(&self) -> bool {
        self.inline_banners_state.open_in_wish_banner.is_some()
    }

    fn close_open_in_wish_banner(&mut self, banner_id: usize) {
        self.model
            .lock()
            .block_list_mut()
    ...
    fn open_in_wish_banner_type_dismissed(
        &self,
    ...
    fn suggest_open_in_wish(
        &mut self,
        openable_path: OpenablePath,
        session: Arc<Session>,
        ctx: &mut ViewContext<Self>,
    ) {
        if self.open_in_wish_banner_type_dismissed(openable_path.file_type, ctx) {
            return;
        }

        // We only show a banner for the most recent command.
        if let Some(prev_state) = &self.inline_banners_state.open_in_wish_banner {
            self.close_open_in_wish_banner(prev_state.id);
        }

        let banner_id = self.inline_banners_state.next_banner_id();
        self.inline_banners_state.open_in_wish_banner = Some(OpenInWishBannerState::new(
            banner_id,
            openable_path,
            session,
        ));
        self.model
            ...
            .append_inline_banner(InlineBannerItem::new(
                banner_id,
                InlineBannerType::OpenInWish,
            ));
        ctx.notify();
    }

    pub fn handle_open_in_wish_banner_action(
        &mut self,
        ...
        match action {
            OpenInWishBannerAction::OpenFile => {
                if let Some(banner_state) = self.inline_banners_state.open_in_wish_banner.take() {
                    ...
                    self.close_open_in_wish_banner(banner_state.id);
                    ctx.notify();
                }
            }
            OpenInWishBannerAction::LearnMore => {
                if let Some(banner_state) = &self.inline_banners_state.open_in_wish_banner {
    ...
    pub fn open_in_wish_banner_accessibility_content(
        &self,
        action: OpenInWishBannerAction,
    ) -> ActionAccessibilityContent {
        match action {
            OpenInWishBannerAction::OpenFile => {
                match &self.inline_banners_state.open_in_wish_banner {
                    Some(banner_state) => {
    ...
    for command in all_parsed_commands(command, escape_char) {
        ...
        if let Some(positionals) = positionals {
            for arg in positionals.iter() {
                ...
                let Some(file_type) = is_file_openable_in_wish(relative_path) else {
                    continue;
                };
