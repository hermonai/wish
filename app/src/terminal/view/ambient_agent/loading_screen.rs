...
use wishui::prelude::{CornerRadius, Radius};
...
use wishui::{AppContext, ModelHandle, SingletonEntity};
...
pub fn render_cloud_mode_loading_screen(
    ...
    let shimmer_element =
        shimmering_wish_loading_text(message, loading_font_size, shimmer_handle.clone(), app);
...
pub fn render_cloud_mode_error_screen(
    ...
    let error_background = wish_core::ui::color::coloru_with_opacity(error_color.into(), 10);
...
pub fn render_cloud_mode_github_auth_required_screen(
    ...
    let auth_icon = ConstrainedBox::new(Icon::Info.to_wishui_icon(accent_color).finish())
        .with_width(ERROR_ICON_SIZE)
        .with_height(ERROR_ICON_SIZE)
        .finish();
    ...
    let auth_background: wishui::elements::Fill = theme.surface_2().into();
...
pub fn render_cloud_mode_cancelled_screen(appearance: &Appearance) -> Box<dyn Element> {
    ...
    let cancelled_icon = ConstrainedBox::new(
        Icon::SlashCircle
            .to_wishui_icon(title_color.into())
            .finish(),
    )
    .with_width(ERROR_ICON_SIZE)
    .with_height(ERROR_ICON_SIZE)
    ...
    // Dark background (surface_2) with subtle border
    let cancelled_background: wishui::elements::Fill = theme.surface_2().into();
