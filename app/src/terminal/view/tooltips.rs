...
fn open_in_wish_tooltip(
    ...
    use crate::{
        ...
        util::tooltips::should_show_open_in_wish_link,
    };
    use settings::Setting as _;
    use wishui::SingletonEntity;

    if !should_show_open_in_wish_link(&path, app) {
        return None;
    }
            ...
            #[cfg(feature = "local_fs")]
            {
                if let GridHighlightedLink::File(file_link) = link {
                    if let Some(path) = file_link.get_inner().absolute_path() {
                        open_in_wish = open_in_wish_tooltip(
                            path.clone(),
                            file_link.get_inner().line_and_column_num,
                            &mut detail,
                            self.mouse_states.open_in_wish_tooltip.clone(),
                            app,
                        );
