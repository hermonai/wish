...
use wish_completer::completer::TopLevelCommandCaseSensitivity;
...
fn test_any_text_file_type_supported() {
    ...
    // .svg is a non-markdown file, and we allow opening it in the Wish Editor (as a "code" file).
    assert_eq!(
        result,
        Some(OpenablePath {
...
fn test_unsupported_binary_file_type() {
    let result = async_io::block_on(check_openable_in_warp(
        "cat app/assets/bundled/png/hermon-logo.png".to_string(),
        Some(workspace_root().to_string_lossy().to_string()),
        TopLevelCommandCaseSensitivity::CaseInsensitive,
        EscapeChar::Backslash,
    ));
