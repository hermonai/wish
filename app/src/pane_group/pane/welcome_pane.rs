...
use wishui::{AppContext, ModelHandle, View, ViewContext, ViewHandle};
...
impl PaneContent for WelcomePane {
    ...
    fn detach(
        ...
        ctx: &mut wishui::ViewContext<PaneGroup>,
    ) {
        let child = self.view.as_ref(ctx).child(ctx);
    ...
    fn has_application_focus(&self, ctx: &mut wishui::ViewContext<PaneGroup>) -> bool {
        self.view.is_self_or_child_focused(ctx)
    }

    fn focus(&self, ctx: &mut wishui::ViewContext<PaneGroup>) {
        self.view
            .as_ref(ctx)
            .child(ctx)
