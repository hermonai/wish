...
use wish_multi_agent_api as multi_agent_api;
...
fn handle_terminal_view_event(
    ...
    if group.pane_contents.contains_key(&pane_id) {
        match event {
            ...
            #[cfg(feature = "local_fs")]
            Event::PreviewCodeInWish { source } => {
                ctx.emit(pane_group::Event::PreviewCodeInWish {
                    source: source.clone(),
                });
            }
            #[cfg(feature = "local_fs")]
...
fn launch_remote_child(
    ...
    let harness_auth_secrets = auth_secret_name
        ...
        .and_then(|name| match orchestration_harness {
            ...
            Harness::Hermon | Harness::OpenCode | Harness::Gemini | Harness::Unknown => None,
        });
