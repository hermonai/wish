...
fn test_expand_directories() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(wishui::r#async::block_on(expand_dirs([directory].into_iter().collect())).debug_print(), "(integration(tests(INTEGRATION_TESTING, data(test, test_launch_config, test_theme, test_theme_with_name, test_workflow))))");
    });
}
