...
const PAYMENT_ISSUE_BANNER_LINE_2_ADMIN_ENTERPRISE: &str =
    "Please contact support@wish.dev to restore access.";
...
impl DriveIndex {
            ...
            if let Some(focused_index) = self.focused_index {
                if Some(&WarpDriveItemId::Space(space)) == self.ordered_items.get(focused_index) {
                    container = container.with_background(
                        wish_core::ui::theme::color::internal_colors::fg_overlay_4(
                            appearance.theme(),
                        ),
                    );
                    is_focused = true;
            ...
            Hoverable::new(
                ...
                move |mouse_state| {
                    ...
                    if mouse_state.is_hovered() && !is_focused || section_state.menu_open {
                        container = container.with_background(
                            wish_core::ui::theme::color::internal_colors::fg_overlay_2(
                                appearance.theme(),
                            ),
                        );
                    }
    ...
    fn render_trash_row(&self, appearance: &Appearance, _: &AppContext) -> Box<dyn Element> {
        ...
        if let Some(focused_index) = self.focused_index {
            if Some(&WarpDriveItemId::Trash) == self.ordered_items.get(focused_index) {
                container = container.with_background(
                    wish_core::ui::theme::color::internal_colors::fg_overlay_4(appearance.theme()),
                );
                is_focused = true;
            }
        }
    ...
    fn render_sorting_button(&self, appearance: &Appearance) -> Box<dyn wishui::Element> {
    ...
    fn render_retry_button(&self, appearance: &Appearance) -> Box<dyn wishui::Element> {
...
impl View for DriveIndex {
    ...
    fn keymap_context(&self, _ctx: &AppContext) -> wishui::keymap::Context {
