...
use wish_core::{report_error, report_if_error};
use wishui::r#async::{FutureExt as _, Timer};
use wishui::{App, Entity, ModelContext, SingletonEntity};
...
impl TelemetryCollector {
    ...
    pub fn flush_telemetry_events_for_shutdown(&self, ctx: &mut ModelContext<TelemetryCollector>) {
        ...
        if execution_mode.send_telemetry_at_shutdown() {
            ...
            match wishui::r#async::block_on(async move {
                server_api
    ...
    fn flush_persisted_events_from_disk(&self, ctx: &mut ModelContext<TelemetryCollector>) {
        ...
        let _ = ctx.spawn(
            async move {
                ...
                let old_path =
                    wish_core::paths::state_dir().join(RUDDER_TELEMETRY_EVENTS_FILE_NAME);
        ...
        let _ = ctx.spawn(
            async move {
                        ...
                        > Utc::now().timestamp()
                {
                    ...
                    {
                        wishui::telemetry::record_app_active_event(
