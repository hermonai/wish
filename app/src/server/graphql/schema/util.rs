...
impl From<GenericStringObjectUniqueKey>
    for wish_graphql::generic_string_object::GenericStringObjectUniqueKey
{
    fn from(key: GenericStringObjectUniqueKey) -> Self {
        use wish_graphql::generic_string_object::GenericStringObjectUniqueKey as GraphQLFormat;
        GraphQLFormat {
            key: key.key,
...
impl From<UniquePer> for wish_graphql::generic_string_object::UniquePer {
    fn from(unique_per: UniquePer) -> Self {
        use wish_graphql::generic_string_object::UniquePer as GraphQLUniquePer;
        match unique_per {
...
impl From<ObjectActionType> for wish_graphql::object_actions::ActionType {
    fn from(action: ObjectActionType) -> Self {
        match action {
            ObjectActionType::Execute => wish_graphql::object_actions::ActionType::Executed,
        }
    }
}
...
fn try_into_object_action_type(
    action_type: wish_graphql::object_actions::ActionType,
) -> Result<ObjectActionType, anyhow::Error> {
    match action_type {
        wish_graphql::object_actions::ActionType::Executed => Ok(ObjectActionType::Execute),
    }
}

/// Converts the graphql action entry (SingleAction, BundledActions) into its ObjectAction corollary.
fn try_into_object_action(
    record: &wish_graphql::object_actions::ActionRecord,
    uid: ObjectUid,
    hashed_sqlite_id: HashedSqliteId,
) -> Result<ObjectAction, anyhow::Error> {
    match record {
        wish_graphql::object_actions::ActionRecord::SingleAction(s) => Ok(ObjectAction {
        ...
        wish_graphql::object_actions::ActionRecord::BundledActions(b) => Ok(ObjectAction {
        ...
        wish_graphql::object_actions::ActionRecord::Unknown => {
...
impl TryInto<ObjectActionHistory> for wish_graphql::object_actions::ObjectActionHistory {
