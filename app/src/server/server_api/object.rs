...
use wish_graphql::{
    ...
    subscriptions::{
        get_wish_drive_updates::GetWarpDriveUpdates, start_graphql_streaming_operation,
    },
};
...
pub trait ObjectClient: 'static + Send + Sync {
    ...
    async fn get_wish_drive_updates(
        &self,
...
impl ObjectClient for ServerApi {
    ...
    async fn get_wish_drive_updates(
        ...
        start_graphql_streaming_operation(
            ...
            |res| {
                res.ok_or_else(|| {
                    anyhow!("missing response data for message in get_wish_drive_updates")
                })
    ...
    async fn untrash_object(&self, id: ServerId) -> Result<ObjectMetadataUpdateResult> {
        ...
        let result = match response.untrash_object {
            wish_graphql::mutations::untrash_object::UntrashObjectResult::UntrashObjectOutput(
                output,
            ) => {
                if output.success {
