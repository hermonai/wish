...
fn test_firebase_token_urls() -> Result<()> {
    ...
    assert_eq!(
        custom_token.proxy_url("https://staging.hermon.ai", "api_key"),
        "https://staging.hermon.ai/proxy/customToken?key=api_key"
    );
    assert_eq!(
        refresh_token.proxy_url("https://staging.hermon.ai", "api_key"),
        "https://staging.hermon.ai/proxy/token?key=api_key"
    );
    Ok(())
}
