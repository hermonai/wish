...
use std::sync::Arc;
use wishui::{Entity, ModelContext, RequestState, SingletonEntity};
...
pub struct Listener {
    ...
    /// so that the user gets a snappier experience once they start using Wish Drive.
...
impl Listener {
        ...
        match event {
            SystemStatsEvent::CpuWasAwakened => {
                ...
                if self.should_subscribe_to_updates {
                    self.get_wish_drive_updates(ctx);
                }
            }
    ...
    fn get_wish_drive_updates(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // TODO: we should investigate having get_wish_drive_updates (and in turn,
