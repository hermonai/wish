    ...
    F: Fn(usize, &mut EventContext, Vector2F) + 'static,
{
    ...
    for (i, session_type) in session_types.iter().enumerate() {
        ...
        let icon = ConstrainedBox::new(
            session_type
                .icon()
                .to_wishui_icon(item_color.into())
                .finish(),
        )
        .with_width(14.)
        .with_height(14.)
...
const ALL_SESSION_TYPES: &[SessionType] = &[SessionType::Hermon, SessionType::Terminal];

/// Returns the session types to display, filtering out Hermon when AI is disabled.
pub fn visible_session_types(show_hermon: bool) -> Vec<SessionType> {
    ALL_SESSION_TYPES
        .iter()
        .filter(|st| show_hermon || !matches!(st, SessionType::Hermon))
        .copied()
        .collect()
}
