...
fn test_editor_display_name() {
    App::test((), |mut app| async move {
        ...
        app.read(|ctx| {
            ...
            assert_eq!(
                &editor_display_name(Some("unknown@wish.dev"), ctx),
                "unknown@wish.dev"
            );

            // If the profile is missing a display name, default to the email.
            assert_eq!(
                &editor_display_name(Some("anon@wish.dev"), ctx),
                "anon@wish.dev"
            );

            // If there's a display name available, use that.
            assert_eq!(
                &editor_display_name(Some("editor@wish.dev"), ctx),
                "The Editor"
            );
        });
    })
