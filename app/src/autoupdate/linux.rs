        ...
        let base_command = match self {
            ...
            PackageManager::Pacman {
                ...
                let repo_prefix = if !is_repo_configured {
                    ...
                    format!("mkdir -p {cache_dir_str}{and}\\\ncp /etc/pacman.conf {cache_dir_str}{and}\\\nsudo sh -c \"echo '\n[{repo_name}]\nServer = https://releases.hermon.ai/linux/pacman/\\$repo/\\$arch' >> /etc/pacman.conf\"{and}\\\n")
                } else {
                    String::new()
                };
                let key_prefix = if !is_signing_key_configured {
                    ...
                    format!("sudo pacman-key -r \"linux-maintainers@wish.dev\" --keyserver hkp://keys.openpgp.org:80{and}\\\nsudo pacman-key --lsign-key \"linux-maintainers@wish.dev\"{and}\\\n")
                } else {
                    String::new()
                };
