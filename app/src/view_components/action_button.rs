...
use wishui::{elements::MainAxisAlignment, Gradient};
use wishui::{elements::MainAxisSize, text_layout::ClipConfig};
use wishui::{
    elements::{
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let mut hoverable = Hoverable::new(self.mouse_state_handle.clone(), |mouse_state| {
            ...
            if let Some(icon) = self.icon {
                ...
                row.add_child(
                    ConstrainedBox::new(icon.to_wishui_icon(icon_fill).finish())
            ...
            if self.has_menu {
                ...
                row.add_child(
                    Container::new(
                        ConstrainedBox::new(
                            Icon::ChevronDown.to_wishui_icon(text_color.into()).finish(),
                        )
                        .with_width(icon_size)
                        .with_height(icon_size)
                        .finish(),
