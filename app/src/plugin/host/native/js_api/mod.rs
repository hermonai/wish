...
cfg_if::cfg_if! {
    if #[cfg(feature = "completions_v2")] {
        ...
        use wish_js::FromWarpJs;
    }
}

/// Returns a JS object representing the Wish Plugin API exposed to external JavaScript plugins.
///
...
/// Returns a JS object representing the Completions namespace for the Wish Plugin API.
///
/// API methods:
///
        ...
        Function::new(
            ...
            MutFn::from(move |val: Value<'js>| {
                if val.is_array() {
                    ...
                    match Vec::<CommandSignature>::from_wish_js(
                        ctx,
                        val,
                    ...
                    let mut plugin = plugin.get_mut();
                    match CommandSignature::from_wish_js(
                        ctx,
                        val,
                        plugin.js_function_registry_mut(),
                    ) {
