...
use crate::util::openable_file_type::{
    resolve_file_target_to_open_in_wish, resolve_file_target_with_editor_choice,
};
use crate::{
...
impl FileTreeView {
    ...
    fn open_file(
        ...
        let target = if editor_layout.is_some() {
            resolve_file_target_to_open_in_wish(path, settings, editor_layout)
        } else {
            ...
            FileTreeItem::File { metadata, .. } => {
                if is_remote {
                    ...
                    if let Some(host_id) = &root_dir.remote_host_id {
                        let remote_path = wish_util::remote_path::RemotePath::new(
                            host_id.clone(),
                            (*metadata.path).clone(),
                        );
                        ctx.emit(FileTreeEvent::OpenFile {
