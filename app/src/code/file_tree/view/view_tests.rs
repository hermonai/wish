...
fn std_path(path: &std::path::Path) -> wish_util::standardized_path::StandardizedPath {
    wish_util::standardized_path::StandardizedPath::try_from_local(path).unwrap()
}
...
fn repo_transition_unregisters_lazy_loaded_path() {
    VirtualFS::test("file_tree_repo_transition", |dirs, mut vfs| {
        ...
        let canonical_repo_root =
            wish_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo_root)
                .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.read(&app, |view, _ctx| {
                assert!(view.registered_lazy_loaded_paths.contains(
                    &wish_util::standardized_path::StandardizedPath::try_from_local(
                        &displayed_root
                    )
                    .unwrap()
                ));
                let displayed_std =
                    wish_util::standardized_path::StandardizedPath::try_from_local(&displayed_root)
                        .unwrap();
                assert_eq!(
            ...
            file_tree_view.read(&app, |view, _ctx| {
                let displayed_std =
                    wish_util::standardized_path::StandardizedPath::try_from_local(&displayed_root)
                        .unwrap();
                let repo_std =
                    wish_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &wish_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &wish_util::standardized_path::StandardizedPath::try_from_local(&src_dir)
                        .unwrap(),
                    ctx,
                );
            });
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &wish_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &wish_util::standardized_path::StandardizedPath::try_from_local(&nested_dir)
                        .unwrap(),
                    ctx,
                );
            });
...
fn click_on_file_under_absorbed_descendant_keeps_file_selected() {
    ...
    VirtualFS::test(
        ...
        |dirs, mut vfs| {
            ...
            let main_rs = wish_server.join("main.rs");
            ...
            App::test((), |mut app| async move {
                ...
                file_tree_view.update(&mut app, |view, ctx| {
                    view.set_is_active(true, ctx);
                    view.set_root_directories(vec![code.clone()], ctx);
                    view.toggle_folder_expansion(&std_path(&code), &std_path(&wish_server), ctx);
                });
                ...
                file_tree_view.update(&mut app, |view, ctx| {
                    view.set_root_directories(vec![wish_server.clone(), code.clone()], ctx);
                });
...
fn pending_focus_target_does_not_re_scroll_after_first_apply() {
    ...
    VirtualFS::test("file_tree_pending_respects_user_scroll", |dirs, mut vfs| {
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.set_is_active(true, ctx);
                view.set_root_directories(vec![wish_server.clone(), tree.clone()], ctx);
            });
            ...
            file_tree_view.read(&app, |view, _ctx| {
                ...
                let root_dir = view.root_directories.get(&std_path(&tree)).unwrap();
                let path = root_dir.items.get(selected.index).unwrap().path();
                assert_eq!(path, &std_path(&wish_server));
                let pending = view.pending_focus_target.as_ref().expect("pending");
    ...
    VirtualFS::test("file_tree_focus_follow_deferred", |dirs, mut vfs| {
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.read(&app, |view, _ctx| {
                ...
                assert_eq!(selected_item.path(), &std_path(&wish_server));
                ...
                assert_eq!(pending.path, std_path(&wish_server));
