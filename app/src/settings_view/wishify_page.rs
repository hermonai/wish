...
use crate::terminal::wishify::settings::{
...
use crate::{
    ...
    terminal::wishify::settings::WishifySettings,
...
const SSH_TMUX_WARPIFICATION_DESCRIPTION: &str = "The tmux ssh wrapper works in many situations where the default one does not, but may require you to hit a button to wishify. Takes effect in new tabs.";
...
impl WishifyPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let wishify_settings_handle = WishifySettings::handle(ctx);

        ctx.observe(&wishify_settings_handle, Self::update_button_states);
        ctx.subscribe_to_model(&wishify_settings_handle, move |me, model, event, ctx| {
        ...
        instance.update_button_states(wishify_settings_handle, ctx);
        instance
    }
    ...
    fn update_button_states(
        &mut self,
        wishify_settings_handle: ModelHandle<WishifySettings>,
        ctx: &mut ViewContext<Self>,
    ) {
        let wishify_settings = wishify_settings_handle.as_ref(ctx);
        self.remove_denylisted_command_button_states = wishify_settings
            .subshell_command_denylist
            .iter()
            .map(|_| Default::default())
            .collect();
        self.remove_added_command_button_states = wishify_settings
            .added_subshell_commands
            .iter()
            .map(|_| Default::default())
            .collect();
        self.remove_denylisted_ssh_button_states = wishify_settings
            .ssh_hosts_denylist
            .iter()
            .map(|_| Default::default())
            .collect();
    ...
    fn handle_added_command_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                WishifySettings::handle(ctx).update(ctx, |wishify_settings, ctx| {
                    wishify_settings.add_subshell_command(new_command, ctx);
                });
    ...
    fn handle_denylisted_command_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                WishifySettings::handle(ctx).update(ctx, |wishify_settings, ctx| {
                    wishify_settings.denylist_subshell_command(new_command, ctx);
                });
    ...
    fn handle_denylisted_ssh_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                WishifySettings::handle(ctx).update(ctx, |wishify_settings, ctx| {
                    wishify_settings.denylist_ssh_host(new_command, ctx);
                });
    ...
    fn remove_denylisted_command(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        WishifySettings::handle(ctx).update(ctx, |wishify, ctx| {
            wishify.remove_denylisted_subshell_command(index, ctx)
        });
    }
    ...
    fn remove_added_command(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        WishifySettings::handle(ctx).update(ctx, |wishify, ctx| {
            wishify.remove_added_subshell_command(index, ctx)
        });
    }
    ...
    fn remove_denylisted_ssh_host(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        WishifySettings::handle(ctx).update(ctx, |wishify, ctx| {
            wishify.remove_denylisted_ssh_host(index, ctx)
        });
    }
}
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            SetSshExtensionInstallMode(mode) => {
                WishifySettings::handle(ctx).update(ctx, |wishify_settings, ctx| {
                    report_if_error!(wishify_settings
                        .ssh_extension_install_mode
                        .set_value(*mode, ctx));
                    send_telemetry_from_ctx!(
...
impl TitleWidget {
    fn render_top_of_page(&self, appearance: &Appearance, _app: &AppContext) -> Box<dyn Element> {
        ...
        Flex::column()
            ...
            .with_child(wishify_description)
            .finish()
    }
}
    ...
    fn render_subshells_section(
        ...
        column.add_child(
            view.build_input_list(
                "Added commands",
                &wishify_settings.added_subshell_commands,
        ...
        column.add_child(
            view.build_input_list(
                "Denylisted commands",
                &wishify_settings.subshell_command_denylist,
            ...
            move || {
                ...
                if enable_ssh_warpification && should_prompt_ssh_tmux_wrapper {
                    ...
                    column.add_child(
                        view.build_input_list(
                            "Denylisted hosts",
                            &wishify_settings.ssh_hosts_denylist,
