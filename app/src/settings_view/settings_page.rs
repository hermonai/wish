...
pub enum SettingsPageViewHandle {
    ...
    HermonCloudAPIKeys(ViewHandle<super::platform_page::PlatformPageView>),
    Privacy(ViewHandle<PrivacyPageView>),
    Wishify(ViewHandle<WishifyPageView>),
...
impl SettingsPageViewHandle {
    pub fn child_view(&self) -> Box<dyn Element> {
        ...
        match self {
            ...
            HermonCloudAPIKeys(view_handle) => ChildView::new(view_handle).finish(),
            Privacy(view_handle) => ChildView::new(view_handle).finish(),
            Wishify(view_handle) => ChildView::new(view_handle).finish(),
        ...
        Container::new(
            Flex::row()
                ...
                .with_child(
                    ConstrainedBox::new(
                        Icon::ChevronRight
                            .to_wishui_icon(appearance.theme().main_text_color(icon_bg))
                            .finish(),
                    )
                    .with_width(16.)
                    .with_height(16.)
...
pub fn render_body_item_label_internal<T: Clone + Action>(
    ...
    if let Some(icon) = label_icon {
        label.add_child(
            Container::new(
                ConstrainedBox::new(icon.to_wishui_icon(label_color).finish())
                    .with_width(16.)
                    .with_height(16.)
                    .finish(),
            )
...
/// [`SettingsWidget`]s.
pub(super) enum PageType<V: wishui::View> {
...
pub(super) enum FilteredPageType<'a, V: wishui::View> {
    Monolith {
...
pub(super) struct Category<V: wishui::View> {
    title: &'static str,
    subtitle: Option<&'static str>,
    widgets: Vec<Box<dyn SettingsWidget<View = V>>>,
}

impl<V: wishui::View> Category<V> {
    pub(super) fn new(
        title: &'static str,
        widgets: Vec<Box<dyn SettingsWidget<View = V>>>,
    ) -> Self {
...
pub(super) struct FilteredCategory<'a, V: wishui::View> {
