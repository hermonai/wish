...
use wishui::id;
use wishui::keymap::{ContextPredicate, Keystroke};
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![ToggleSettingActionPair::custom(
            SettingActionPairDescriptions::new(
                "Show Hermon Agent changelog in new agent conversation view",
...
impl SettingsWidget for AgentAttributionWidget {
    ...
    fn render(
        ...
        Flex::column()
            ...
            .with_child(render_ai_setting_description(
                "Hermon Agent can add attribution to commit messages and pull requests it creates",
                !state.is_disabled,
                app,
            ))
            .finish()
...
impl SettingsWidget for CloudAgentComputerUseWidget {
    ...
    fn search_terms(&self) -> &str {
        "hermon cloud agent computer use orchestration multi-agent"
    }
...
impl SettingsWidget for CloudHandoffWidget {
    ...
    fn should_render(&self, _app: &AppContext) -> bool {
        FeatureFlag::HermonHandoff.is_enabled() && FeatureFlag::HandoffLocalCloud.is_enabled()
    }
...
impl ApiKeysWidget {
    ...
    fn render_custom_inference_description(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let text_fragments = vec![
            FormattedTextFragment::plain_text(
                "Use your own API keys from model providers for Wish Agent. You can also add custom endpoints to use third-party models. Custom endpoints must support the OpenAI-compatible Chat Completions API. API keys are stored locally and are never synced to the cloud. Using auto models or models from providers you have not provided API keys for will consume Wish credits. ",
            ),
            FormattedTextFragment::hyperlink(
                "Learn more",
...
impl AwsBedrockWidget {
    ...
    fn render_aws_bedrock_section(
        ...
        fn render_credential_status_card(
            ...
            let icon = Container::new(
                ConstrainedBox::new(icon.to_wishui_icon(title_color).finish())
                    .with_width(16.)
                    .with_height(16.)
                    .finish(),
            )
