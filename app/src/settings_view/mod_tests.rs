...
fn subpage_display_names_are_correct() {
    assert_eq!(SettingsSection::WarpAgent.to_string(), "Wish Agent");
...
fn subpage_from_str_parses_display_names() {
    // Both the legacy "Hermon" name and the new "Wish Agent" display name must
    ...
    assert_eq!(
        SettingsSection::from_str("Wish Agent"),
        Ok(SettingsSection::WarpAgent)
    );
    assert_eq!(
...
fn auto_select_jumps_away_from_filtered_out_subpage() {
    ...
    // The nav order: Hermon, Profiles, ..., Knowledge, ThirdPartyCLI
...
fn collapsed_umbrella_with_filtered_subpages_uses_first_visible_subpage() {
    ...
    let stops = build_nav_stops(&nav_items, |section| {
        // Hide WishAgent (first AI subpage); keep the rest.
