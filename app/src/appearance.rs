...
mod macos_app_icon {
    ...
    pub use wish_core::channel::{Channel, ChannelState};
    pub use wishui::platform::mac::{make_nsstring, AutoreleasePoolGuard};
...
pub use wish_core::ui::appearance::{Appearance, AppearanceEvent};
...
impl AppearanceManager {
    ...
    pub fn set_app_icon(&self, app: &AppContext) {
        ...
        unsafe {
            ...
            let plugin_name = make_nsstring("WishDockTilePlugin.docktileplugin");
            let plugin_path: id =
...
fn load_default_monospace_font_family(ctx: &mut AppContext) -> anyhow::Result<FamilyId> {
    wishui::fonts::Cache::handle(ctx).update(ctx, |font_cache, _| {
...
fn get_or_load_font_family(font_name: &str, ctx: &mut AppContext) -> Option<FamilyId> {
    wishui::fonts::Cache::handle(ctx).update(ctx, |font_cache, _| {
        match font_cache.get_or_load_system_font(font_name) {
            Ok(family) => {
                let font_id = font_cache.select_font(family, wishui::fonts::Properties::default());
