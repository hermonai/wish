...
use wish_completer::completer::CommandExitStatus;
...
/// 2. Begin codebase indexing for all repositories (Hermon harness only).
...
pub fn prepare_environment(
    ...
    async move {
        ...
        // Only index the codebase for the Hermon harness; third-party harnesses (e.g. Claude)
        // have their own methods for navigating a codebase.
        let should_index_codebase = harness == Harness::Hermon;
...
async fn prepare_environment_impl(
    ...
    if !github_repos.is_empty() {
        ...
        let should_wait_for_indexing = !matches!(
            wish_isolation_platform::detect(),
            Some(
                wish_isolation_platform::IsolationPlatformType::DockerSandbox
                    | wish_isolation_platform::IsolationPlatformType::Namespace
            )
        );
