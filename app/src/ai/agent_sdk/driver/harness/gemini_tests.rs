...
fn prepare_gemini_settings_adds_context_file_name_when_system_prompt_present() {
    ...
    assert_eq!(
        ...
        Value::Array(vec![Value::String("HERMON_SYSTEM_PROMPT.md".to_owned())]),
    );
}
...
fn prepare_gemini_settings_appends_to_existing_context_file_name() {
    ...
    let file_names: Vec<String> = settings["context"]["fileName"]
        ...
        .collect();
    assert_eq!(file_names, vec!["existing.md", "HERMON_SYSTEM_PROMPT.md"]);
}
...
fn prepare_gemini_settings_does_not_duplicate_system_prompt_file_name() {
    ...
    fs::write(
        &settings_path,
        r#"{"context":{"fileName":["HERMON_SYSTEM_PROMPT.md"]}}"#,
    )
    .unwrap();
    ...
    let file_names: Vec<String> = settings["context"]["fileName"]
        ...
        .map(|v| v.as_str().unwrap().to_owned())
        .collect();
    assert_eq!(file_names, vec!["HERMON_SYSTEM_PROMPT.md"]);
}
