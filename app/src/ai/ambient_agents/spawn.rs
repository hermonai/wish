...
fn poll_run_until_joinable_session(
    ...
    async_stream::stream! {
        ...
        let mut timeout_timer = FutureExt::fuse(match timeout {
            Some(d) => wishui::r#async::Timer::after(d),
            None => wishui::r#async::Timer::never(),
        });
        let mut last_state = None;
        loop {
            let mut poll_timer = FutureExt::fuse(wishui::r#async::Timer::after(TASK_STATUS_POLL_INTERVAL));
