...
use wish_core::ui::color::contrast::{
    foreground_color_with_minimum_contrast, MinimumAllowedContrast,
};
use wish_core::ui::color::Rgb;
use wish_core::ui::theme::{Fill, WarpTheme};
use wishui::elements::{Highlight, HighlightedRange, Text};
...
fn create_secret_gray_highlight() -> Highlight {
    Highlight::new().with_text_style(
        TextStyle::new()
            .with_foreground_color(wishui::color::ColorU::new(128, 128, 128, 255))
...
fn add_slash_command_highlight(
    ...
    if let Some(existing) = existing_highlight {
        ...
        bold_properties.weight = wishui::fonts::Weight::Bold;
...
pub fn render_autonomy_dropdown_setting_speedbump_footer<A>(
    ...
    dropdown: &wishui::ViewHandle<crate::view_components::dropdown::Dropdown<A>>,
    settings_link_handle: MouseStateHandle,
    app: &AppContext,
) -> Box<dyn Element>
where
    A: wishui::Action + Clone,
{
    let appearance = Appearance::as_ref(app);
    let theme = appearance.theme();
    Flex::row()
        ...
        .with_child(wishui::elements::ChildView::new(dropdown).finish())
        .with_child(
            Expanded::new(
                1.,
                Align::new(
