...
use wishui::elements::new_scrollable::{NewScrollable, ScrollableAppearance, SingleAxisConfig};
use wishui::elements::{
...
use wishui::platform::{Cursor, LineStyle};
...
fn render_hover_card(
    ...
    let card = Container::new(column.finish())
        ...
        .with_background(bg)
        .with_border(wishui::elements::Border::all(1.).with_border_fill(outline))
...
fn render_status_badge(
    ...
    let icon_el = ConstrainedBox::new(icon.to_wishui_icon(color.into()).finish())
        .with_width(12.)
        .with_height(12.)
        .finish();
    let label = Text::new(
...
fn render_chip(
    ...
    let icon_el = ConstrainedBox::new(icon.to_wishui_icon(icon_color.into()).finish())
        .with_width(12.)
        .with_height(12.)
        .finish();
    let text = Text::new(
...
fn render_pill(
    ...
    self_terminal_view_id: wishui::EntityId,
    app: &AppContext,
) -> Box<dyn Element> {
    let appearance = Appearance::as_ref(app);
    let theme = appearance.theme();
    ...
    // `crates/wishui_core/src/elements/hoverable.rs`). We can therefore move
    ...
    let pill_body = Hoverable::new(mouse_state, move |hover_state| {
        ...
        let leading: Box<dyn Element> = match (is_pinned, status.as_ref()) {
            (true, _) => ConstrainedBox::new(Icon::Pin.to_wishui_icon(text_color.into()).finish())
...
fn render_overflow_button(
    ...
    let button = Hoverable::new(mouse_state, move |hover_state| {
        ...
        let icon = ConstrainedBox::new(Icon::DotsVertical.to_wishui_icon(text_color).finish())
...
fn render_avatar_disc(
    ...
    let glyph_element: Box<dyn Element> = match glyph {
        ...
        AvatarGlyph::Icon(icon) => {
            ConstrainedBox::new(icon.to_wishui_icon(theme.background()).finish())
...
pub fn render_orchestration_breadcrumbs(
    ...
    let chevron = ConstrainedBox::new(
        Icon::ChevronRight
            .to_wishui_icon(chevron_color.into())
            .finish(),
    )
    .with_width(16.)
    .with_height(16.)
