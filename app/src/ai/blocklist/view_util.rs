...
/// Label we use for the the command palette action to create a new local Hermon agent pane.
        ...
        if state.is_hovered() {
            ...
            let tool_tip = appearance
                ...
                .with_style(UiComponentStyles {
                    font_size: Some(12.),
                    background: Some(wishui::elements::Fill::Solid(tooltip_background)),
...
fn render_input_icon(icon: Icon, color: Fill, app: &AppContext) -> Box<dyn Element> {
    ...
    ConstrainedBox::new(
        Container::new(icon.to_wishui_icon(color).finish())
    ...
    F: FnMut(&mut EventContext) + 'static,
{
    ...
    let mut label_children = vec![ConstrainedBox::new(icon.to_wishui_icon(color).finish())
