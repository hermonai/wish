...
fn test_format_log_entry_preserves_stdout_and_stderr_sections() {
    let output = CommandOutput {
        stdout: b"https://github.com/hermonai/wish-internal/pull/123\n".to_vec(),
        stderr: b"warning output\n".to_vec(),
        status: CommandExitStatus::Success,
        exit_code: Some(wish_core::command::ExitCode::from(0)),
    };
    ...
    assert!(entry.contains(
        "stdout:\n<<<STDOUT\nhttps://github.com/hermonai/wish-internal/pull/123\n>>>STDOUT"
    ));
    assert!(entry.contains("stderr:\n<<<STDERR\nwarning output\n>>>STDERR"));
}
