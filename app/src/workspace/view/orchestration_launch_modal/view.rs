...
const LEARN_MORE_URL: &str = "https://hermon.ai/placeholder-launch-blog-link";
...
const FEATURE_ITEMS: &[FeatureItem] = &[
    FeatureItem {
        ...
        description: "Use Hermon Cloud to spin up Claude Code or Codex agents in the cloud; Hermon helps you track and steer the agents.",
        badge: None,
    },
    FeatureItem {
        icon: Icon::Atom02,
        title: "Multi-agent orchestration",
        description: "Wish Agents orchestrate swarms of subagents, allowing you to parallelize tasks.",
        badge: None,
    },
    FeatureItem {
        icon: Icon::Cognition,
...
pub struct OrchestrationLaunchModal {
    ...
    go_to_wish_button: ViewHandle<ActionButton>,
}
...
impl OrchestrationLaunchModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let go_to_wish_button = ctx.add_view(|_ctx| {
    ...
    fn render_description(appearance: &Appearance) -> Box<dyn Element> {
        Text::new(
            "We've made major improvements to Wish's cloud agent orchestration platform, Hermon Cloud.",
            appearance.ui_font_family(),
            14.,
        )
    ...
    fn render_feature_row(&self, item: &FeatureItem, appearance: &Appearance) -> Box<dyn Element> {
        let icon_el = ConstrainedBox::new(
            item.icon
                .to_wishui_icon(Fill::Solid(modal_text_sub(appearance)))
                .finish(),
        )
        .with_width(16.)
        .with_height(16.)
    ...
    fn render_body(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let footer = Flex::row()
            ...
            .with_child(
                Expanded::new(1., ChildView::new(&self.go_to_wish_button).finish()).finish(),
            )
            .finish();
