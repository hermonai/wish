...
pub use util::*;
use wishui::integration::PersistedDataMap;
pub use wishui::integration::RUNTIME_TAG_FAILURE_REASON;
use wishui::{App, SingletonEntity as _, WindowId};
    ...
    BlocklistAIHistoryModel::handle(app).update(app, |history_model, _| {
        ...
        let debug_link = conversation
            ...
            .map(|token| {
                ...
                // with staging.hermon.ai instead of localhost:8080.
                token
                    .debug_link()
                    .replace("host.docker.internal:8080", "staging.hermon.ai")
            })
