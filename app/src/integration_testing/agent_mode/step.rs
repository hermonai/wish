...
pub fn hydrate_ai_conversation(file_name: &str) -> TestStep {
    ...
    let Ok(request) = wish_multi_agent_api::Request::decode(file_bytes) else {
...
) -> impl FnMut(&mut wishui::App, wishui::WindowId) -> wishui::integration::AssertionOutcome {
    |app, window_id| {
        use crate::BlocklistAIHistoryModel;
        use wishui::integration::AssertionOutcome;
        let terminal_view = terminal_view(app, window_id, 0, 0);
