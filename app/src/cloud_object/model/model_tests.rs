...
fn test_object_editor_timeout() {
    App::test((), |mut app| async move {
        ...
        CloudModel::handle(&app).update(&mut app, |model, _ctx| {
            ...
            notebook.metadata.current_editor_uid = Some("ian@wish.dev".to_string());
        });
...
/// way, the folders containing the updated object move to the top of the Wish Drive index if it's
